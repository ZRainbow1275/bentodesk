...
//! animation-crate split), the `Lerp` trait now lives in `bento-nano-style`
//! alongside the concrete types it operates on. `bento-nano-tree::animation`
//! re-exports it for backward compatibility — existing call sites that
//! import `bento_nano_tree::Lerp` continue to compile unchanged.
//!
