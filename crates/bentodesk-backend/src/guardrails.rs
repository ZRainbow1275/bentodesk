...
//! reached for `crate::config::settings::AppSettings` directly; the native
...
/// `SafetyProfile`; the native port uses the same variants so on-disk
...
/// `&LayoutData` and reached for `zone.items.len()`; the native port accepts
