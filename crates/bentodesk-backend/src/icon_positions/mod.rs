...
//! | 1.x                                     | native                                                                      |
//! |-----------------------------------------|---------------------------------------------------------------------------|
//! | `tauri::AppHandle` for path resolution  | caller passes `&Path` for the data dir; native backend has no AppHandle     |
//! | `crate::storage::state_data_dir(handle)`| caller resolves via `bentodesk_backend::storage::state_data_dir(...)`    |
...
/// does not yet exist in the native backend) to keep T-086 self-contained per
...
mod tests {
    ...
    fn tempdir() -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("bentodesk-icon-pos-{pid}-{n}"));
