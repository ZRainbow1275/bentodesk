...
//! them to [`bentodesk_theme::ThemeTokens`] for the renderer.
//!
...
use bentodesk_theme::typo::{FontSizes, FontWeights, LineHeights};
use bentodesk_theme::{PaletteTokens, ThemeTokens, TypoTokens, radius, shadow, spacing};
...
/// In native, the caller (typically `bentodesk-app::storage`) owns app-data
...
/// / `border`); the native [`PaletteTokens`] has 16 slots. We derive the missing
/// slots by darkening / lightening the four canonical colors, matching the
/// hand-tuned dark-default ratios used in `bentodesk_theme::palette::DARK`.
///
