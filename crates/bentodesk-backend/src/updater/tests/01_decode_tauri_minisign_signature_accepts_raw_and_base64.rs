...
fn check_reads_local_manifest_and_returns_available_update() {
    let manifest_path = std::env::temp_dir().join(format!(
        "bentodesk-update-manifest-{}.json",
        std::process::id()
    ));
    std::fs::write(
        &manifest_path,
...
fn download_copies_local_artifact_and_emits_progress_ready() {
    let manifest_path = std::env::temp_dir().join(format!(
        "bentodesk-update-download-manifest-{}.json",
        std::process::id()
    ));
    let artifact_path = std::env::temp_dir().join(format!(
        "bentodesk-update-artifact-{}.bin",
        std::process::id()
    ));
...
fn download_copies_tauri_v2_platform_artifact_and_emits_progress_ready() {
    let manifest_path = std::env::temp_dir().join(format!(
        "bentodesk-update-platform-manifest-{}.json",
        std::process::id()
    ));
    let artifact_path = std::env::temp_dir().join(format!(
        "bentodesk-update-platform-artifact-{}.bin",
        std::process::id()
    ));
    std::fs::write(&artifact_path, b"installer-bytes").expect("write artifact");
    let artifact_source = artifact_path.to_string_lossy().replace('\\', "/");
...
fn download_accepts_valid_minisign_signature_with_sha256() {
    let manifest_path = std::env::temp_dir().join(format!(
        "bentodesk-update-signed-sha-manifest-{}.json",
        std::process::id()
    ));
    let artifact_path = std::env::temp_dir().join(format!(
        "bentodesk-update-signed-sha-artifact-{}.bin",
        std::process::id()
    ));
...
fn download_accepts_valid_minisign_signature_only_manifest() {
    let manifest_path = std::env::temp_dir().join(format!(
        "bentodesk-update-signature-only-manifest-{}.json",
        std::process::id()
    ));
    let artifact_path = std::env::temp_dir().join(format!(
        "bentodesk-update-signature-only-artifact-{}.bin",
        std::process::id()
    ));
    std::fs::write(&artifact_path, b"test").expect("write artifact");
    let artifact_source = artifact_path.to_string_lossy().replace('\\', "/");
...
fn download_accepts_tauri_base64_minisign_signature() {
    let manifest_path = std::env::temp_dir().join(format!(
        "bentodesk-update-base64-signature-manifest-{}.json",
        std::process::id()
    ));
    let artifact_path = std::env::temp_dir().join(format!(
        "bentodesk-update-base64-signature-artifact-{}.bin",
        std::process::id()
    ));
    std::fs::write(&artifact_path, b"test").expect("write artifact");
    let artifact_source = artifact_path.to_string_lossy().replace('\\', "/");
...
fn download_verifies_tauri_v2_platform_minisign_signature() {
    let manifest_path = std::env::temp_dir().join(format!(
        "bentodesk-update-platform-signature-manifest-{}.json",
        std::process::id()
    ));
    let artifact_path = std::env::temp_dir().join(format!(
        "bentodesk-update-platform-signature-artifact-{}.bin",
        std::process::id()
    ));
    std::fs::write(&artifact_path, b"test").expect("write artifact");
    let artifact_source = artifact_path.to_string_lossy().replace('\\', "/");
...
fn download_verifies_tauri_v2_platform_base64_minisign_signature() {
    let manifest_path = std::env::temp_dir().join(format!(
        "bentodesk-update-platform-base64-signature-manifest-{}.json",
        std::process::id()
    ));
    let artifact_path = std::env::temp_dir().join(format!(
        "bentodesk-update-platform-base64-signature-artifact-{}.bin",
        std::process::id()
    ));
    std::fs::write(&artifact_path, b"test").expect("write artifact");
    let artifact_source = artifact_path.to_string_lossy().replace('\\', "/");
...
fn download_deletes_stage_and_emits_error_when_minisign_mismatches() {
    let manifest_path = std::env::temp_dir().join(format!(
        "bentodesk-update-signature-mismatch-manifest-{}.json",
        std::process::id()
    ));
    let artifact_path = std::env::temp_dir().join(format!(
        "bentodesk-update-signature-mismatch-artifact-{}.bin",
        std::process::id()
    ));
    std::fs::write(&artifact_path, b"Test").expect("write artifact");
    let artifact_source = artifact_path.to_string_lossy().replace('\\', "/");
...
fn download_deletes_stage_and_emits_error_when_base64_signature_is_invalid() {
    let manifest_path = std::env::temp_dir().join(format!(
        "bentodesk-update-invalid-base64-signature-manifest-{}.json",
        std::process::id()
    ));
    let artifact_path = std::env::temp_dir().join(format!(
        "bentodesk-update-invalid-base64-signature-artifact-{}.bin",
        std::process::id()
    ));
    std::fs::write(&artifact_path, b"test").expect("write artifact");
    let artifact_source = artifact_path.to_string_lossy().replace('\\', "/");
...
fn download_deletes_stage_and_emits_error_when_signed_sha256_mismatches() {
    let manifest_path = std::env::temp_dir().join(format!(
        "bentodesk-update-sha-mismatch-manifest-{}.json",
        std::process::id()
    ));
    let artifact_path = std::env::temp_dir().join(format!(
        "bentodesk-update-sha-mismatch-artifact-{}.bin",
        std::process::id()
    ));
    std::fs::write(&artifact_path, b"test").expect("write artifact");
    let artifact_source = artifact_path.to_string_lossy().replace('\\', "/");
...
fn background_check_emits_available_and_preserves_pending_download() {
    let manifest_path = std::env::temp_dir().join(format!(
        "bentodesk-update-background-manifest-{}.json",
        std::process::id()
    ));
    let artifact_path = std::env::temp_dir().join(format!(
        "bentodesk-update-background-artifact-{}.bin",
        std::process::id()
    ));
    std::fs::write(&artifact_path, b"installer-bytes").expect("write artifact");
    let artifact_source = artifact_path.to_string_lossy().replace('\\', "/");
