//! T-087 — single-pass hierarchical clustering over file FeatureVectors.
//!
...
//! ## Q2-aligned cleanup
//!
//! 1.x's `regex_escape` helper + `format!("^{}", regex_escape(prefix))`
//! emission is gone. The `regex` crate is no longer on the §8 whitelist
//! and `Condition::NameMatchesRegex` was retired by Q2; the recommender
//! now emits a bare-prefix `AutoGroupRule.pattern`, matching the
//! `suggestions::suggest_groups` convention.

use std::collections::HashMap;

use serde::{Deserialize, Serialize};

use super::feature::{FeatureVector, featurize, mean_pairwise_similarity, similarity};
...
///
/// **Q2**: pattern emission no longer carries the `^` regex anchor or
/// runs through `regex_escape` — `AutoGroupRule.pattern` is a plain
/// case-insensitive prefix string the matcher in `commands::grouping`
/// already handles by case-folded `starts_with`.
    ...
    for c in clusters {
        ...
        let rule = if let Some(ext) = dominant_ext {
            ...
            AutoGroupRule {
                ...
                pattern: Some(name.to_lowercase()), // Q2: bare prefix
                extensions: None,
            }
        };
...
mod tests {
    ...
    /// Synthesize an RFC3339 timestamp `day` days after the epoch using
    /// the native time helper (chrono is forbidden by spec §8.1).
    fn rfc3339_for_day(day: i64) -> String {
        let st = UNIX_EPOCH + Duration::from_secs((day * 86_400) as u64);
    ...
    fn mk_file(name: &str, ext: &str, day: i64, size: u64) -> FileInfo {
        FileInfo {
            ...
            created_at: rfc3339_for_day(20_000 + day),
            is_directory: false,
    ...
    fn respects_max_cluster_size() {
        let files: Vec<FileInfo> = (0..30)
            .map(|i| mk_file(&format!("Report_{i}.pdf"), "pdf", i, 1000))
            .collect();
        let clusters = cluster(&files);
        for c in &clusters {
            assert!(
    ...
    #[test]
    fn name_pattern_emission_carries_no_regex_anchor() {
        // Cluster of mixed extensions sharing a common prefix → name-pattern path.
        let files = vec![
            mk_file("Alpha_Report.pdf", "pdf", 0, 1000),
            mk_file("Alpha_Notes.txt", "txt", 0, 1000),
            mk_file("Alpha_Data.csv", "csv", 0, 1000),
            mk_file("Alpha_Plan.md", "md", 0, 1000),
        ];
        let suggestions = clusters_to_suggestions(&files);
        for s in suggestions {
            ...
            if s.rule.rule_type == GroupRuleType::NamePattern {
                ...
                assert!(!pat.contains('\\'), "Q2: no regex escapes, got {pat:?}");
            }
        }
    }
}
