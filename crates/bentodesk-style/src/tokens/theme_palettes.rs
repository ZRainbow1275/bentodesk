...
//! tables captured in the native migration's `m6-theme-tokens.md`. Alpha byte
//! = `round(alpha * 255)` (e.g. 0.55 → 0x8C,
//! 0.82 → 0xD1) — round, never truncate, to match the existing `PALETTE_DARK`
//! comment math.
//!
//! ## The 4 native-only slots (no Tauri source field)
//!
...
//!     (minibar is global chrome, not per-theme-tinted in current native).
//!
//! ## §8 / §10
//!
//! Every entry is a `pub const PaletteTauri` (`Copy`, no allocation, no new
//! crate dep). The leaf style crate stays free of `bentodesk-theme`.
...
/// dark page and became white-on-white in a native auxiliary HWND.  Native keeps
