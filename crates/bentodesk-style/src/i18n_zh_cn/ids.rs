...
// 未来集成验证 sections (native-only, absent from Tauri) were deleted in
...
// Replaces the bespoke native "使用模式 (速度模式)" (id 117) with the real
...
// native `SettingsUpdaterStatus` variants to one of these labels; the action
...
// native shell has no event listener, so it exposes an explicit Refresh
// button that fires `ListSettingsBackups` — the native equivalent of Tauri's
// auto-refresh.
...
// hint here; native has no existing equivalent string, so this id is appended
...
// textarea + Import/Export) is deferred (no native keyboard/text-input infra
...
// `stackDetachMember`; the remaining labels are native's selected-stack
