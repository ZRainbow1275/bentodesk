...
use bento_nano_style::{Color, Edges, Length, StringId};
...
pub struct TextNode {
    ...
    /// `bento_nano_style::t(id)` and ignores `content`. When `None`, the
...
mod tests {
    use super::*;
    use bento_nano_style::{ZH_CN, i18n_zh_cn::ids as zh_ids, init_locale};
