...
//! `APP_ROOT` `AtomicPtr`. Per
//! `~/.claude/projects/.../memory/feedback_rust_analyzer_false_positive.md`,
//! the unsafe raw-pointer cast may make rust-analyzer noisy — `cargo build`
//! is the truth source.
...
fn startup_diag_skip(flag: &str) -> bool {
    startup_diag_skip_value(
        std::env::var("BENTODESK_NANO_DIAG_SKIP").ok().as_deref(),
        flag,
    )
}
...
struct AppRoot {
    ...
    live_folder_events: crossbeam_channel::Receiver<bento_nano_backend::watcher::ZoneRefreshEvent>,
...
fn detected_default_locale() -> &'static bento_nano_style::LookupTable {
    ...
    if tag.starts_with("zh") {
        &bento_nano_style::ZH_CN
    } else {
        &bento_nano_style::EN_US
    }
}
