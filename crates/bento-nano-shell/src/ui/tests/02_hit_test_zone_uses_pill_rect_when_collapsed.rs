...
fn settings_hit_resolves_buttons_and_body() {
    ...
    let lang = bento_nano_app::settings_panel::settings_language_chip_rect(app.viewport, scroll_y);
    assert_eq!(
...
fn m1g_settings_hit_resolves_backup_create_and_per_row_restore() {
    ...
    let body = bento_nano_app::settings_panel::settings_body_rect(app.viewport);
    let label =
        bento_nano_app::settings_panel::settings_backup_label_rect(app.viewport, scroll_y, &flags);
    assert!(
