...
fn icon_picker_pointer_commit_updates_zone_and_persists_restart_equivalent() {
    ...
    let loaded = bento_nano_platform::storage::read_zones(&zones_path)
...
fn bulk_manager_text_button_starts_editor_and_enter_dispatches_update() {
    ...
    let text = bento_nano_app::business::bulk_manager_panel::BULK_MANAGER_ACTION_BUTTONS
        .iter()
        .find(|spec| {
            spec.hit
                == bento_nano_app::business::bulk_manager_panel::BulkManagerPointerHit::TextEdit
        })
        .copied()
