...
fn scratch_zones_path(label: &str) -> std::path::PathBuf {
    ...
    root.push(format!("bento-nano-capsule-{label}-{}", std::process::id()));
...
fn test_app_root() -> AppRoot {
    ...
    AppRoot {
        ...
        timeline_buffer: std::cell::RefCell::new(
            bento_nano_backend::timeline::TimelineBuffer::default(),
        ),
    }
}
...
fn rect_center(rect: bento_nano_style::Rect) -> (f32, f32) {
...
fn tooltip_checkpoint_meta(
    id: &'static str,
    summary: &'static str,
) -> bento_nano_backend::timeline::CheckpointMeta {
    bento_nano_backend::timeline::CheckpointMeta {
