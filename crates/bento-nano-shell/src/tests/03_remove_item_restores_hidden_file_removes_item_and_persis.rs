...
fn item_drag_out_reports_visible_outcomes() {
    ...
    assert!(start_item_drag_out_with(&root, path.clone(), |files| {
        ...
        Ok(bento_nano_backend::drag_drop::DragOutcome::Dropped)
    }));
    ...
    assert!(start_item_drag_out_with(&root, path.clone(), |_files| {
        Ok(bento_nano_backend::drag_drop::DragOutcome::Cancelled)
    }));
    assert_eq!(
        root.app
            .borrow()
    ...
    assert!(start_item_drag_out_with(&root, path, |_files| {
        Err(bento_nano_backend::drag_drop::DragDropError::NoFiles)
    }));
    assert_eq!(
        root.app
            .borrow()
