...
fn recovery_bundle_tamper_rejects_before_mutating_live_zones() {
    ...
    let data_root = bento_nano_backend::recovery_bundle::data_root_for_state_file(&zones_path)
        .expect("data root");
    let mut bundle = bento_nano_backend::recovery_bundle::load_bundle(&data_root)
        .expect("load bundle")
        .expect("bundle exists");
    bundle.zones_bin_b64 = bento_nano_backend::config_vault::wire::base64_encode(b"tampered");
    bento_nano_backend::recovery_bundle::write_bundle(&data_root, &bundle)
...
fn sample_rule(id: &str) -> bento_nano_backend::rules::Rule {
