...
fn rules_state_dir_uses_zones_parent_directory() {
    let dir = std::env::temp_dir().join("bento-nano-rules-parent-test");
...
fn rules_save_stamps_empty_id_and_persists_json() {
    let dir = std::env::temp_dir().join(format!("bento-nano-rules-save-{}", std::process::id()));
...
fn live_folder_refresh_rebuilds_zone_items_and_preserves_tags() {
    let root = test_app_root();
    let scratch =
        std::env::temp_dir().join(format!("bento-nano-live-folder-{}", std::process::id()));
...
fn live_folder_refresh_no_change_reports_visible_main_status_without_dirtying() {
    ...
    let scratch = std::env::temp_dir().join(format!(
        "bento-nano-live-folder-no-change-{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&scratch);
    std::fs::create_dir_all(&scratch).expect("scratch");
...
fn live_folder_items_participate_in_rules_in_zone_conditions() {
    ...
    let plan = bento_nano_backend::rules::executor::build_plan(
        &rule,
...
fn rules_move_to_zone_rejects_live_folder_target_without_mutating() {
    let root = test_app_root();
    let scratch = std::env::temp_dir().join(format!(
        "bento-nano-live-folder-move-reject-{}",
        std::process::id()
    ));
...
fn rules_run_plan_move_to_zone_mutates_live_zone_state() {
    let root = test_app_root();
    let scratch =
        std::env::temp_dir().join(format!("bento-nano-rules-run-zone-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&scratch);
    std::fs::create_dir_all(&scratch).expect("scratch");
...
fn rules_tag_action_updates_live_and_persisted_item_metadata() {
    let root = test_app_root();
    let scratch = std::env::temp_dir().join(format!("bento-nano-rules-tag-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&scratch);
    std::fs::create_dir_all(&scratch).expect("scratch");
...
fn rules_run_records_timeline_pre_and_post_checkpoints() {
    let root = test_app_root();
    let scratch =
        std::env::temp_dir().join(format!("bento-nano-rules-timeline-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&scratch);
    std::fs::create_dir_all(&scratch).expect("scratch");
...
fn rule_execution_timeline_coalesces_same_trigger_and_key() {
    let root = test_app_root();
    let scratch = std::env::temp_dir().join(format!(
        "bento-nano-rules-timeline-coalesce-{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&scratch);
    std::fs::create_dir_all(&scratch).expect("scratch");
