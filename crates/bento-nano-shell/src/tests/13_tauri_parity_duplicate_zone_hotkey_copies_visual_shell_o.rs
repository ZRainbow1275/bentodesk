...
fn keybinding_setting_applies_runtime_table_and_rejects_conflicts() {
    ...
    assert!(apply_hotkey_setting_to_runtime(
        ...
        &bento_nano_app::SettingValue::Str(SmolStr::new_static("Ctrl+Shift+T")),
    ));
    assert!(!apply_hotkey_setting_to_runtime(
        &root,
...
fn keybinding_reset_removes_persisted_override() {
    ...
    let mut vault = bento_nano_backend::config_vault::Vault::open(&vault_path).expect("open vault");
    vault.set_setting(
        "keybinding.timeline.open",
        bento_nano_backend::config_vault::SettingValue::Str(SmolStr::new_static("Ctrl+Shift+T")),
    );
    ...
    let reopened =
        bento_nano_backend::config_vault::Vault::open(&vault_path).expect("reopen vault");
...
fn snapshot_delete_rejects_path_shaped_ids() {
    ...
    let manager = bento_nano_backend::layout::SnapshotManager::new(snapshot_dir);
...
fn recovery_bundle_capture_restore_round_trips_live_zones() {
    ...
    ));
    {
        ...
        bento_nano_platform::storage::write_zones_atomic(&app.zones_path, &app.zones)
            .expect("persist restored zones");
    }
    let persisted = bento_nano_platform::storage::read_zones(&zones_path)
...
fn recovery_bundle_capture_restore_round_trips_safety_manifest() {
    ...
    let manifest = bento_nano_backend::stealth::SafetyManifest {
        schema_version: bento_nano_backend::stealth::MANIFEST_SCHEMA_VERSION.to_string(),
        entries: vec![bento_nano_backend::stealth::ManifestEntry {
    ...
    bento_nano_backend::stealth::save_manifest(&hidden_dir, &manifest).expect("save manifest");
    {
        let mut app = root.app.borrow_mut();
        app.zones_path = zones_path.clone();
    ...
    let restored = bento_nano_backend::stealth::load_manifest(&hidden_dir).expect("load restored");
...
fn recovery_bundle_capture_restore_round_trips_icon_backup_sidecar() {
    ...
    bento_nano_backend::icon_positions::persist_to_file(&layout, &data_root)
    ...
    let outcome = restore_recovery_icon_backup_with(
        ...
        |saved_layout| {
            ...
            Ok(bento_nano_backend::icon_positions::RestoreResult {
                restored: 1,
                skipped: 0,
                failed: 0,
    ...
    let restored = bento_nano_backend::icon_positions::load_from_file(&data_root)
...
fn recovery_diagnostics_export_writes_visible_bundle_report() {
    ...
    let report_path = bento_nano_backend::recovery_bundle::diagnostics_path(state_dir);
    ...
    let persisted = bento_nano_backend::storage::read_json_with_recovery::<
        bento_nano_backend::recovery_bundle::RecoveryDiagnosticsReport,
