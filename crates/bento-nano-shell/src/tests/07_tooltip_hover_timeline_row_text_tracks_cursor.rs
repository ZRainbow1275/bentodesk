...
fn tooltip_hover_timeline_row_text_tracks_cursor() {
    ...
    let (row_x, row_y) = {
        ...
        rect_center(bento_nano_app::business::timeline::panel::timeline_row_rect(app.viewport, 1))
    };

    let unselected = {
        let app = root.app.borrow();
        tooltip_command_for_timeline_hover(&app, bento_nano_app::WindowHandle::NULL, row_x, row_y)
    };
    match unselected {
...
fn tooltip_hover_snapshot_picker_button_producer_queues_show() {
    ...
    let (load_x, load_y) = {
        let app = root.app.borrow();
        rect_center(
            bento_nano_app::business::timeline::snapshot_picker::snapshot_picker_button_rect(
                app.viewport,
                load,
            ),
        )
...
fn tooltip_hover_snapshot_picker_row_text_tracks_cursor() {
    ...
    let (row_x, row_y) = {
        let app = root.app.borrow();
        rect_center(
            bento_nano_app::business::timeline::snapshot_picker::snapshot_picker_row_rect(
                app.viewport,
                1,
            ),
        )
...
fn tooltip_hover_icon_picker_text_tracks_selected_slot_and_hide() {
    ...
    let hide = {
        let app = root.app.borrow();
        tooltip_command_for_icon_picker_hover(&app, bento_nano_app::WindowHandle::NULL, 0.0, 0.0)
    };
...
fn tooltip_hover_suggestor_omits_row_bubble_but_keeps_action_help() {
    ...
    let row_show = {
        let app = root.app.borrow();
        tooltip_command_for_suggestor_hover(&app, bento_nano_app::WindowHandle::NULL, row_x, row_y)
    };
    assert!(
        row_show.is_none(),
    ...
    let hide = {
        let app = root.app.borrow();
        tooltip_command_for_suggestor_hover(&app, bento_nano_app::WindowHandle::NULL, 0.0, 0.0)
    };
    assert!(matches!(hide, Some(Command::HideTooltip)));
}
