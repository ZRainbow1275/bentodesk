...
fn tooltip_snapshot(
    id: &'static str,
    name: &'static str,
) -> bento_nano_backend::layout::DesktopSnapshot {
    bento_nano_backend::layout::DesktopSnapshot {
...
fn tooltip_hover_search_row_producer_queues_show_and_hide() {
    let root = test_app_root();
    {
        ...
        let mut rows =
            smallvec::SmallVec::<[bento_nano_app::business::search_bar::SearchHit; 8]>::new();
        rows.push(bento_nano_app::business::search_bar::SearchHit {
            id: SmolStr::new_static("zone:77"),
            kind: bento_nano_backend::search::SearchItemKind::Zone,
    ...
    let (row_x, row_y) = {
        let app = root.app.borrow();
        rect_center(bento_nano_app::business::search_bar::search_row_rect(
            app.viewport,
            0,
        ))
    };
    ...
    let show = {
        let app = root.app.borrow();
        tooltip_command_for_search_hover(&app, bento_nano_app::WindowHandle::NULL, row_x, row_y)
    };
    match show {
        Some(Command::ShowTooltip { anchor, text }) => {
            assert_eq!(anchor, bento_nano_app::WindowHandle::NULL);
    ...
    let hide = {
        let app = root.app.borrow();
        tooltip_command_for_search_hover(&app, bento_nano_app::WindowHandle::NULL, 0.0, 0.0)
    };
...
fn tooltip_hover_search_close_producer_queues_close_text() {
    ...
    let (close_x, close_y) = {
        let app = root.app.borrow();
        rect_center(bento_nano_app::business::search_bar::search_close_rect(
            app.viewport,
        ))
    };
    ...
    let show = {
        let app = root.app.borrow();
        tooltip_command_for_search_hover(&app, bento_nano_app::WindowHandle::NULL, close_x, close_y)
    };
    match show {
...
fn tooltip_hover_bulk_manager_button_producer_queues_show_and_hide() {
    ...
    let update_spec = bento_nano_app::business::bulk_manager_panel::BULK_MANAGER_ACTION_BUTTONS
        .iter()
        .copied()
        .find(|spec| {
    ...
    let (update_x, update_y) = {
        let app = root.app.borrow();
        rect_center(
            bento_nano_app::business::bulk_manager_panel::bulk_manager_button_rect(
                app.viewport,
                update_spec,
            ),
        )
    ...
    let hide = {
        let app = root.app.borrow();
        tooltip_command_for_bulk_manager_hover(&app, bento_nano_app::WindowHandle::NULL, 0.0, 0.0)
    };
    assert!(matches!(hide, Some(Command::HideTooltip)));
}
...
fn tooltip_hover_bulk_manager_row_text_tracks_selection_state() {
    ...
    let (row_x, row_y) = {
        let app = root.app.borrow();
        rect_center(
            bento_nano_app::business::bulk_manager_panel::bulk_manager_row_rect(app.viewport, 0),
        )
    };
...
fn tooltip_hover_rules_wizard_row_text_tracks_cursor_and_hide() {
    ...
    let hide = {
        let app = root.app.borrow();
        tooltip_command_for_rules_wizard_hover(&app, bento_nano_app::WindowHandle::NULL, 0.0, 0.0)
    };
    assert!(matches!(hide, Some(Command::HideTooltip)));
}
...
fn tooltip_hover_timeline_button_producer_queues_show_and_hide() {
    ...
    let (save_x, save_y) = {
        let app = root.app.borrow();
        rect_center(
            bento_nano_app::business::timeline::panel::timeline_button_rect(app.viewport, save),
        )
    };

    let show = {
        let app = root.app.borrow();
        tooltip_command_for_timeline_hover(&app, bento_nano_app::WindowHandle::NULL, save_x, save_y)
    };
    match show {
        Some(Command::ShowTooltip { text, .. }) => {
    ...
    let hide = {
        let app = root.app.borrow();
        tooltip_command_for_timeline_hover(&app, bento_nano_app::WindowHandle::NULL, 0.0, 0.0)
    };
    assert!(matches!(hide, Some(Command::HideTooltip)));
}
