...
pub(super) fn restore_recovery_vault_payload_to_vault(
    ...
    let _validated = bento_nano_backend::config_vault::Vault::open(&temp_path)?;
    ...
    let restored = bento_nano_backend::config_vault::Vault::open(&vault_path)?;
    *vault = restored;
    Ok(())
}

pub(super) fn restore_recovery_vault_payload(
    payload: &bento_nano_backend::recovery_bundle::RecoveredVaultPayload,
) -> Result<(), RecoveryBundleShellError> {
    let Some(mtx) = bento_nano_backend::config_vault::Vault::global() else {
...
pub(super) fn recovery_manifest_snapshots(
    ...
    Vec<bento_nano_backend::recovery_bundle::RecoverySafetyManifest>,
    RecoveryBundleShellError,
> {
    ...
    for desktop_path in desktop_paths {
        ...
        let manifest = bento_nano_backend::stealth::load_manifest(&manifest_dir)?;
        let config = bento_nano_backend::stealth::StealthConfig {
            desktop_path: SmolStr::new(desktop_path.to_string_lossy().as_ref()),
            app_data_dir: SmolStr::new(app_data_dir.to_string_lossy().as_ref()),
        };
        let expected_manifest_dir = bento_nano_backend::stealth::hidden_dir_for(&config)?;
        if expected_manifest_dir != manifest_dir {
            continue;
        }
        snapshots.push(
            bento_nano_backend::recovery_bundle::RecoverySafetyManifest {
...
pub(super) fn restore_recovery_safety_manifests(
    manifests: &[bento_nano_backend::recovery_bundle::RecoverySafetyManifest],
) -> Result<(), RecoveryBundleShellError> {
    for snapshot in manifests {
        bento_nano_backend::stealth::persist_manifest_snapshot(
...
pub(super) fn recovery_icon_backup(
    data_root: &Path,
) -> Result<Option<bento_nano_backend::icon_positions::SavedIconLayout>, RecoveryBundleShellError> {
    bento_nano_backend::icon_positions::load_from_file(data_root).map_err(Into::into)
}

pub(super) fn restore_recovery_icon_backup_with<F>(
    data_root: &Path,
    icon_backup: Option<&bento_nano_backend::icon_positions::SavedIconLayout>,
    restore_layout: F,
) -> RecoveryIconRestoreOutcome
where
    F: FnOnce(
        &bento_nano_backend::icon_positions::SavedIconLayout,
    ) -> Result<
        bento_nano_backend::icon_positions::RestoreResult,
        bento_nano_backend::icon_positions::IconPositionError,
    >,
{
    ...
    if let Err(error) = bento_nano_backend::icon_positions::persist_to_file(icon_backup, data_root)
    {
...
pub(super) fn restore_recovery_icon_backup(
    ...
    restore_recovery_icon_backup_with(
        data_root,
        icon_backup,
        bento_nano_backend::icon_positions::restore_layout,
    )
}

pub(super) fn capture_recovery_bundle(
    root: &AppRoot,
) -> Result<bento_nano_backend::recovery_bundle::RecoveryBundleSummary, RecoveryBundleShellError> {
    ...
    let data_root = bento_nano_backend::recovery_bundle::data_root_for_state_file(&zones_path)?;
    ...
    let user_data_files = bento_nano_backend::recovery_bundle::collect_user_data_files(&data_root)?;
    bento_nano_backend::recovery_bundle::refresh_bundle_with_user_data(
        &data_root,
        &zones_path,
        &zones_bin,
        zone_count,
        bento_nano_backend::recovery_bundle::RecoveryBundleSidecars {
            vault: vault_payload,
...
pub(super) fn restore_recovery_bundle(
    ...
    let data_root = bento_nano_backend::recovery_bundle::data_root_for_state_file(&zones_path)?;
    let payload = bento_nano_backend::recovery_bundle::recover_zones_payload(&data_root)?
    ...
    let user_data_restore = bento_nano_backend::recovery_bundle::restore_user_data_files(
        &data_root,
...
pub(super) fn export_recovery_diagnostics(
    root: &AppRoot,
) -> Result<bento_nano_backend::recovery_bundle::RecoveryDiagnosticsReport, RecoveryBundleShellError>
{
    let zones_path = {
        let app = root.app.borrow();
        if app.zones_path.as_os_str().is_empty() {
    ...
    let data_root = bento_nano_backend::recovery_bundle::data_root_for_state_file(&zones_path)?;
    bento_nano_backend::recovery_bundle::export_diagnostics_report(&data_root)?
        .ok_or(RecoveryBundleShellError::MissingBundle)
}
