...
pub(super) fn settings_backup_retained_count(
    ...
    match vault.get_setting("backup.max_retained") {
        Some(bento_nano_backend::config_vault::SettingValue::Int(value)) => usize::try_from(value)
            .ok()
...
pub(super) fn create_settings_backup_from_vault(
    ...
    vault.set_setting(
        ...
        bento_nano_backend::config_vault::SettingValue::Str(SmolStr::new(backup_id)),
    );
    vault.flush()?;
...
pub(super) fn restore_settings_backup_by_id_from_vault(
    ...
    bento_nano_backend::config_vault::Vault::open(&selected_path)?;
    ...
    let mut restored = bento_nano_backend::config_vault::Vault::open(&vault_path)?;
    restored.set_setting(
        SETTING_BACKUP_LAST_RESTORED,
        bento_nano_backend::config_vault::SettingValue::Str(selected_entry.id.clone()),
    );
    restored.flush()?;
    *vault = restored;
