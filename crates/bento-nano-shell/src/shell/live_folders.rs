...
pub(super) fn live_folder_items_from_path(
    ...
    bento_nano_backend::watcher::validate_folder(folder).map_err(|error| error.to_string())?;
    ...
    for (index, (_name, path)) in entries.into_iter().enumerate() {
        ...
        let icon_hash = load_icon_hash_for_path(&bento_nano_app::ItemPath::new(&path_string))
...
pub(super) fn bind_zone_to_folder(
    ...
    bento_nano_backend::watcher::bind(zone_id, &folder).map_err(|error| error.to_string())?;
    let changed = {
        let mut app = root.app.borrow_mut();
        let Some(zone) = app.zones.get_mut(zone_id) else {
            let _ = bento_nano_backend::watcher::unbind(zone_id);
            return Err(format!("live folder zone not found: {}", zone_id.0));
        };
...
pub(super) fn unbind_zone_folder(root: &AppRoot, zone_id: ZoneId) -> Result<bool, String> {
    bento_nano_backend::watcher::unbind(zone_id).map_err(|error| error.to_string())?;
    let mut app = root.app.borrow_mut();
    let Some(zone) = app.zones.get_mut(zone_id) else {
        return Err(format!("live folder zone not found: {}", zone_id.0));
    };
