    ...
    match msg {
            ...
            unsafe {
                ...
                if !p.is_null() {
                    ...
                    if slot.kind == WindowKind::Main {
                        let (x, y, w, h) = bento_nano_platform::main_window_rect();
                        SetWindowPos(
                            hwnd,
                            ptr::null_mut(),
                            x,
        ...
        WM_NCHITTEST => {
            ...
            unsafe {
                ...
                let app = root.app.borrow();
                let dpi = slot.state.dpi.get();
                let lx = bento_nano_style::dpi::device_to_logical_f32(pt.x as f32, dpi);
                let ly = bento_nano_style::dpi::device_to_logical_f32(pt.y as f32, dpi);
