...
pub(super) fn run() {
    ...
    bento_nano_style::init_locale(default_locale);
    ...
    log_static(if bento_nano_platform::dcomp::acrylic_feature_enabled() {
    ...
    let ole_drop_ready = if startup_diag_skip("ole") {
        ...
        match bento_nano_backend::drag_drop::initialize_ole() {
            Ok(()) => true,
            Err(e) => {
                tracing::warn!(
    ...
    let desktop_sources = bento_nano_backend::desktop_sources::all_desktop_dirs(None);
    ...
    if !startup_diag_skip("live_folder")
        && let Err(e) = bento_nano_backend::watcher::ensure_initialised(live_folder_event_tx)
    {
        tracing::warn!(
        ...
        if let Some(dir) = zones_path.parent() {
            ...
            if startup_diag_skip("vault") {
                tracing::info!(
                    ...
                    "BENTODESK_NANO_DIAG_SKIP=vault; config vault startup skipped"
                );
            } else if let Err(e) = bento_nano_backend::config_vault::init_global(&vault_path) {
                tracing::warn!(
            ...
            let icon_config = bento_nano_backend::icon::IconConfig {
            ...
            if !startup_diag_skip("rules") {
                bento_nano_backend::rules::scheduler::spawn(
                    dir.to_path_buf(),
    ...
    // `bento_nano_platform::main_window_rect` returns the live primary work
    ...
    let (main_x, main_y, main_w, main_h) = bento_nano_platform::main_window_rect();
    ...
    let acrylic_runtime = match bento_nano_platform::dcomp::device() {
        Ok(_) => bento_nano_platform::dcomp::acrylic_runtime_available(),
        Err(_) => None,
    };
    ...
    if ole_drop_ready {
        if let Err(e) = bento_nano_backend::drag_drop::register_drop_target(
            hwnd,
            ole_drop_can_accept,
            ole_drop_commit,
        ) {
    ...
    message_loop::run();
    if ole_drop_ready {
        bento_nano_backend::drag_drop::uninitialize_ole();
    }
}
