...
pub(super) fn auto_organize_desktop(root: &AppRoot) -> bool {
    ...
    let suggestions = bento_nano_backend::grouping::suggest_groups(&files);
...
pub(super) fn load_icon_hash_for_path(path: &bento_nano_app::ItemPath) -> Option<String> {
    ...
    match bento_nano_backend::icon::protocol::extract_and_cache(&cache, path.0.as_str()) {
        Ok(hash) => {
            tracing::debug!(
                target: "bentodesk::icon",
                path = %path.0,
