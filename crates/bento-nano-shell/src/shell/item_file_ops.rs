...
pub(super) fn rename_item_file(
    ...
    let icon_hash =
        load_icon_hash_for_path(&bento_nano_app::ItemPath::new(effective_path.as_str()))
...
pub(super) fn move_hidden_item_file_between_zones(
    ...
    let zone_dir = match bento_nano_backend::stealth::zone_hidden_dir_for(
        &config,
        &to_zone_id.0.to_string(),
    ) {
        Ok(dir) => dir,
        Err(e) => {
            tracing::warn!(
...
pub(super) fn stealth_config_for_source(
    root: &AppRoot,
    source_path: &str,
) -> Option<bento_nano_backend::stealth::StealthConfig> {
    ...
    Some(bento_nano_backend::stealth::StealthConfig {
