...
pub(super) fn capture_current_timeline_snapshot(root: &AppRoot, name: &str) -> DesktopSnapshot {
    ...
    DesktopSnapshot {
        ...
        captured_at: SmolStr::new(bento_nano_backend::time::now_rfc3339()),
    }
}
...
pub(super) fn bento_zones_from_app(app: &AppState) -> Vec<BentoZone> {
    ...
    let now = bento_nano_backend::time::now_rfc3339();
    app.zones
        .iter()
        .enumerate()
        .map(|(index, zone)| {
...
///
/// Behaviour:
/// - Decimal u64 strings (legacy nano persistence) pass through unchanged.
