...
pub(super) fn queue_add_items(
    ...
    for file in files {
        root.dispatcher.push(Command::AddItem(
            zone_id,
            bento_nano_app::ItemPath::new(file),
        ));
    }
    log_static(
        format!(
...
pub(super) fn zone_for_client_device_point(
    ...
    let x = bento_nano_style::dpi::device_to_logical_f32(device_x as f32, dpi);
    let y = bento_nano_style::dpi::device_to_logical_f32(device_y as f32, dpi);
...
pub(super) fn zone_for_screen_point(
    root: &AppRoot,
    hwnd: HWND,
    point: bento_nano_backend::drag_drop::DropPoint,
) -> Option<ZoneId> {
...
pub(super) fn ole_drop_can_accept(point: bento_nano_backend::drag_drop::DropPoint) -> bool {
...
pub(super) fn ole_drop_commit(point: bento_nano_backend::drag_drop::DropPoint, files: Vec<String>) {
    let Some(root) = app_root() else {
        return;
    };
    if root.item_drag_out_active.get() {
