...
use bento_nano_app::AppState;
use bento_nano_platform::storage;
...
fn unique_tmp(name: &str) -> PathBuf {
    ...
    let nanos = std::time::SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    p.push(format!("bento-nano-persist-{name}-{pid}-{nanos}.bin"));
    p
}
        ...
        if let Ok(entries) = std::fs::read_dir(parent) {
            for e in entries.flatten() {
                if let Some(name) = e.file_name().to_str() {
                    if name.contains(".corrupt-") && name.contains("bento-nano-persist-corrupt") {
