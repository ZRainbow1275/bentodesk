...
//! moved into `bentodesk-style::lerp` so the orphan rule is satisfied for
//! third-crate impls (Color/Rect/Size/Length all live in style). Tree
//! re-exports the trait so existing `bentodesk_tree::Lerp` call sites keep
//! compiling unchanged.
//!
//! Spec §C4: animation state lives in `bentodesk-tree` (not a separate crate)
