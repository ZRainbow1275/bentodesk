...
//! `EffectId` minted by the caller via [`bento_nano_tree::create_effect`].
...
use bento_nano_tree::{EffectArena, EffectId, NodeId};
...
/// [`bento_nano_tree::create_effect`] beforehand, attaching `node` as a
/// tracked node so [`EffectArena::flush`] folds it into the per-frame
/// [`bento_nano_tree::DirtyDigest`].
...
mod tests {
    use super::*;
    use bento_nano_tree::{DirtyDigest, Effect, create_effect};
    use core::cell::Cell;
    use std::rc::Rc;
