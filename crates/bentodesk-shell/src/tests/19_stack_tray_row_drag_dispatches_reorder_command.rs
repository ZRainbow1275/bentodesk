...
fn locale_setting_command_persists_display_locale_key() {
    assert_eq!(
        ...
        Command::SetSetting {
            ...
            value: bentodesk_app::SettingValue::Str(smol_str::SmolStr::new_static("en-US")),
        }
    );
}
...
fn updater_frequency_setting_uses_tauri_wire_key() {
    assert_eq!(
        ...
        Command::SetSetting {
            ...
            value: bentodesk_app::SettingValue::Str(smol_str::SmolStr::new_static("Manual")),
        }
    );
    assert_eq!(
...
fn settings_bool_commands_use_stable_dotted_keys() {
    assert_eq!(
        ...
        Command::SetSetting {
            ...
            value: bentodesk_app::SettingValue::Bool(false),
        }
    );
    assert_eq!(
        ...
        Command::SetSetting {
            ...
            value: bentodesk_app::SettingValue::Bool(true),
        }
    );
    assert_eq!(
...
fn encryption_mode_setting_uses_stable_dotted_key_and_safe_modes_only() {
    assert_eq!(
        ...
        Command::SetSetting {
            ...
            value: bentodesk_app::SettingValue::Str(smol_str::SmolStr::new_static("Dpapi")),
        }
    );
    ...
    assert_eq!(
        ...
        Command::SetSetting {
            key: smol_str::SmolStr::new_static("encryption.mode"),
            value: bentodesk_app::SettingValue::Str(smol_str::SmolStr::new_static("None")),
        }
    );
    assert_eq!(
...
fn setting_runtime_apply_updates_visible_state_only_for_valid_payloads() {
    ...
    assert!(apply_setting_value_to_app(
        ...
        &bentodesk_app::SettingValue::Str(smol_str::SmolStr::new_static("Daily")),
    ));
    assert_eq!(
        app.update_check_frequency.get(),
        UpdateCheckFrequency::Daily
    ...
    assert!(apply_setting_value_to_app(
        ...
        &bentodesk_app::SettingValue::Bool(false),
    ));
    assert!(!app.update_auto_download.get());
    ...
    assert!(apply_setting_value_to_app(
        ...
        &bentodesk_app::SettingValue::Bool(true),
    ));
    ...
    assert!(apply_setting_value_to_app(
        ...
        &bentodesk_app::SettingValue::Str(smol_str::SmolStr::new_static("Dpapi")),
    ));
    ...
    assert!(!apply_setting_value_to_app(
        &app,
        SETTING_UPDATES_CHECK_FREQUENCY,
        &bentodesk_app::SettingValue::Str(smol_str::SmolStr::new_static("Hourly")),
    ));
    assert!(!apply_setting_value_to_app(
        &app,
        SETTING_ENCRYPTION_MODE,
        &bentodesk_app::SettingValue::Str(smol_str::SmolStr::new_static("Passphrase")),
    ));
}
...
fn theme_base_accent_validates_applies_persists_and_clears() {
    ...
    path.push(format!(
        "bentodesk-theme-base-accent-{}.vault",
        std::process::id()
    ));
...
fn settings_accent_vault_persist_writes_and_clears_modern_and_legacy_keys() {
    let mut path = std::env::temp_dir();
    path.push(format!(
        "bentodesk-settings-accent-clear-{}.vault",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&path);
    let mut vault = Vault::open(&path).expect("open test vault");
...
fn active_theme_loads_applies_persists_and_rejects_unknown() {
    ...
    );
    {
        ...
        assert_eq!(
            ...
            bentodesk_style::tokens::PALETTE_OCEAN_BLUE
        );
    }
