...
fn settings_plugin_commands_list_toggle_and_uninstall_real_registry_entries() {
  ...
  "glassmorphism": {
    ...
    let reloaded = bentodesk_backend::plugins::PluginRegistry::load(state_dir).expect("registry");
    assert_eq!(
        reloaded
...
fn settings_install_plugin_command_extracts_archive_and_refreshes_theme_providers() {
    ...
    let registry = bentodesk_backend::plugins::PluginRegistry::load(state_dir).expect("registry");
...
fn zone_display_mode_setting_uses_tauri_wire_key_and_applies() {
    ...
    assert!(apply_setting_value_to_app(
        ...
        &bentodesk_app::SettingValue::Str(SmolStr::new_static("click")),
    ));
    assert_eq!(app.zone_display_mode.get(), ZoneDisplayMode::Click);
    assert!(!apply_setting_value_to_app(
        &app,
        SETTING_ZONE_DISPLAY_MODE,
        &bentodesk_app::SettingValue::Str(SmolStr::new_static("popup")),
    ));
    assert_eq!(app.zone_display_mode.get(), ZoneDisplayMode::Click);

    let mut path = std::env::temp_dir();
    path.push(format!(
        "bentodesk-zone-display-mode-{}.vault",
        std::process::id()
    ));
...
fn encryption_mode_setting_changes_real_vault_mode_before_runtime_apply() {
    let mut path = std::env::temp_dir();
    path.push(format!(
        "bentodesk-encryption-mode-{}.vault",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&path);
    let mut vault = Vault::open(&path).expect("open test vault");
    ...
    assert!(
        persist_setting_to_vault(
            ...
            &bentodesk_app::SettingValue::Str(smol_str::SmolStr::new_static("None")),
        )
        .expect("persist none")
    );
...
fn passphrase_encryption_command_path_encrypts_and_verifies_vault() {
    let mut path = std::env::temp_dir();
    path.push(format!(
        "bentodesk-passphrase-mode-{}.vault",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&path);
    let mut vault = Vault::open(&path).expect("open test vault");
...
fn locked_passphrase_vault_rejects_plain_setting_until_unlock_succeeds() {
    let mut path = std::env::temp_dir();
    path.push(format!(
        "bentodesk-passphrase-unlock-{}.vault",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&path);
    {
        ...
        vault.set_setting(
            "display.locale",
            bentodesk_backend::config_vault::SettingValue::Str(smol_str::SmolStr::new_static(
                "zh-CN",
            )),
        );
        persist_passphrase_to_vault(&mut vault, "correct horse").expect("persist passphrase");
    ...
    assert!(
        persist_setting_to_vault(
            ...
            &bentodesk_app::SettingValue::Str(smol_str::SmolStr::new_static("en-US")),
        )
        .is_err()
    );
...
fn settings_backup_create_now_writes_real_backup_file() {
    ...
    dir.push(format!("bentodesk-settings-backup-{}", std::process::id()));
