...
fn bulk_manager_pointer_row_click_toggles_selection() {
    ...
    let row = bentodesk_app::business::bulk_manager_panel::bulk_manager_row_rect(
...
fn bulk_manager_search_input_filters_rows_without_triggering_shortcuts() {
    ...
    let search = bentodesk_app::business::bulk_manager_panel::bulk_manager_search_rect(
        root.app.borrow().viewport,
    );
...
fn bulk_manager_sort_header_click_toggles_sort_key_and_row_order() {
    ...
    ));
    {
        ...
        assert_eq!(
            ...
            bentodesk_app::business::bulk_manager_panel::SortDirection::Ascending
        );
    ...
    ));
    {
        ...
        assert_eq!(
            manager.sort_direction(),
            bentodesk_app::business::bulk_manager_panel::SortDirection::Descending
        );
        let ids: Vec<ZoneId> = manager.visible_rows().iter().map(|row| row.id).collect();
        assert_eq!(ids, vec![ZoneId(1), ZoneId(2)]);
        assert_eq!(
...
fn bulk_manager_pointer_update_button_dispatches_bulk_update() {
    ...
    let update = bentodesk_app::business::bulk_manager_panel::BULK_MANAGER_ACTION_BUTTONS
        .iter()
        .find(|spec| {
...
fn bulk_manager_icon_button_opens_non_zone_icon_picker() {
    ...
    let icon = bentodesk_app::business::bulk_manager_panel::BULK_MANAGER_ACTION_BUTTONS
        .iter()
        .find(|spec| {
            spec.hit
                == bentodesk_app::business::bulk_manager_panel::BulkManagerPointerHit::IconPicker
        })
        .copied()
