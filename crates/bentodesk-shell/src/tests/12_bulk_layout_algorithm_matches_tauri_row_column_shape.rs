...
fn timeline_manual_checkpoint_persists_to_real_store() {
    ...
    let loaded = bentodesk_backend::timeline::CheckpointStore::new(timeline_dir)
...
fn timeline_pointer_row_click_selects_and_loads_checkpoint() {
    ...
    let _ = save_timeline_checkpoint(&root, None, Some(smol_str::SmolStr::new_static("second")))
        .expect("second checkpoint");
    let row =
        bentodesk_app::business::timeline::panel::timeline_row_rect(root.app.borrow().viewport, 1);
...
fn timeline_pointer_save_button_dispatches_manual_checkpoint() {
    ...
    let save = bentodesk_app::business::timeline::panel::TIMELINE_ACTION_BUTTONS
        ...
        .copied()
        .expect("save button");
    let rect = bentodesk_app::business::timeline::panel::timeline_button_rect(
...
fn timeline_pointer_restore_button_dispatches_selected_checkpoint() {
    ...
    let restore = bentodesk_app::business::timeline::panel::TIMELINE_ACTION_BUTTONS
        .iter()
        .find(|spec| {
            spec.hit == bentodesk_app::business::timeline::panel::TimelinePointerHit::Restore
        })
        .copied()
...
fn snapshot_picker_save_load_delete_round_trip() {
    ...
    let manager = bentodesk_backend::layout::SnapshotManager::new(snapshot_dir.clone());
...
fn snapshot_picker_pointer_row_click_selects_snapshot() {
    ...
    let row = bentodesk_app::business::timeline::snapshot_picker::snapshot_picker_row_rect(
        root.app.borrow().viewport,
        1,
    );
...
fn snapshot_picker_pointer_save_button_dispatches_snapshot_command() {
    ...
    let save = bentodesk_app::business::timeline::snapshot_picker::SNAPSHOT_PICKER_ACTION_BUTTONS
            .iter()
            .find(|spec| {
                spec.hit
                    == bentodesk_app::business::timeline::snapshot_picker::SnapshotPickerPointerHit::Save
            })
            .copied()
            .expect("save button");
    let rect = bentodesk_app::business::timeline::snapshot_picker::snapshot_picker_button_rect(
        root.app.borrow().viewport,
        save,
    );
...
fn snapshot_picker_pointer_delete_button_uses_two_step_confirmation() {
    ...
    let snapshot = save_layout_snapshot(&root, Some(smol_str::SmolStr::new_static("Delete me")))
        .expect("save snapshot");
    let delete = bentodesk_app::business::timeline::snapshot_picker::SNAPSHOT_PICKER_ACTION_BUTTONS
            .iter()
            .find(|spec| {
                spec.hit
                    == bentodesk_app::business::timeline::snapshot_picker::SnapshotPickerPointerHit::Delete
            })
            .copied()
...
fn snapshot_picker_pointer_timeline_button_dispatches_open_timeline() {
    ...
    let timeline =
            bentodesk_app::business::timeline::snapshot_picker::SNAPSHOT_PICKER_ACTION_BUTTONS
                .iter()
                .find(|spec| {
                    spec.hit
                        == bentodesk_app::business::timeline::snapshot_picker::SnapshotPickerPointerHit::Timeline
                })
                .copied()
