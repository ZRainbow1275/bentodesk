...
fn rule_execution_timeline_keeps_distinct_coalesce_keys() {
    ...
    let scratch = std::env::temp_dir().join(format!(
        "bentodesk-rules-timeline-distinct-{}",
        std::process::id()
    ));
...
fn rules_run_stats_increment_persisted_rule() {
    let dir = std::env::temp_dir().join(format!("bentodesk-rules-stats-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).expect("scratch");
    bentodesk_backend::rules::save_all(&dir, &[sample_rule("rule-stats")]).expect("seed rule");
...
fn rules_on_file_change_event_runs_matching_rule_for_changed_file_only() {
    ...
    bentodesk_backend::rules::save_all(&scratch, &[rule]).expect("seed rule");
    ...
    let loaded = bentodesk_backend::rules::load_all(&scratch);
    assert_eq!(loaded[0].run_count, 1);
    assert!(loaded[0].last_run.is_some());
...
fn rules_on_file_change_event_does_not_run_on_demand_rules() {
    let root = test_app_root();
    let scratch = std::env::temp_dir().join(format!(
        "bentodesk-rules-file-change-on-demand-{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&scratch);
    std::fs::create_dir_all(&scratch).expect("scratch");
...
fn rules_interval_scheduler_event_runs_due_interval_rule() {
    let root = test_app_root();
    let scratch =
        std::env::temp_dir().join(format!("bentodesk-rules-interval-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&scratch);
    std::fs::create_dir_all(&scratch).expect("scratch");
...
fn rules_interval_scheduler_event_ignores_non_interval_rule() {
    let root = test_app_root();
    let scratch = std::env::temp_dir().join(format!(
        "bentodesk-rules-interval-on-demand-{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&scratch);
    std::fs::create_dir_all(&scratch).expect("scratch");
