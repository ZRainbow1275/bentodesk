...
pub(super) fn search_tooltip_text_for_hit(
    ...
    match hit {
        SearchBarPointerHit::Close => {
            let label = bentodesk_style::t(bentodesk_style::i18n_zh_cn::ids::SEARCH_CLOSE);
...
pub(super) fn bulk_manager_row_tooltip_text(app: &AppState, row_index: usize) -> Option<SmolStr> {
    ...
    let selection_action = if bentodesk_style::current_locale_is(&bentodesk_style::ZH_CN) {
        if selected { "取消选择" } else { "选择" }
    } else if selected {
        "Deselect"
    } else {
    ...
    let item_label = if bentodesk_style::current_locale_is(&bentodesk_style::ZH_CN) {
...
pub(super) fn rules_wizard_condition_row_tooltip_text(
    ...
    let predicate = if bentodesk_style::current_locale_is(&bentodesk_style::ZH_CN) {
