...
pub(super) fn dispatch(
    ...
    match command {
        ...
        Command::SetThemeBase(accent) => match bentodesk_backend::config_vault::Vault::global() {
        ...
        Command::SetActiveTheme(theme_id) => {
            ...
            match validation_error {
                None => match bentodesk_backend::config_vault::Vault::global() {
                    Some(mtx) => match mtx.lock() {
                        Ok(mut vault) => {
                ...
                Ok(imported) => {
                    ...
                    match load_theme_selection_for_root(root, imported_id.as_str()) {
                        Ok((options, theme)) => {
                            ...
                            match bentodesk_backend::config_vault::Vault::global() {
                                Some(mtx) => match mtx.lock() {
                ...
                Ok(manifest) => match refresh_plugin_dependent_state(root) {
                    Ok(_changed) => {
                        ...
                        set_plugin_setting_success(
                            root,
                            localized_plugin_message(
                                bentodesk_style::i18n_zh_cn::ids::PLUGIN_STATUS_INSTALLED_PREFIX,
                                manifest.name,
                            ),
                        );
                        effects.needs_redraw = true;
                ...
                Ok(plugin) => match refresh_plugin_dependent_state(root) {
                    Ok(_changed) => {
                        ...
                        set_plugin_setting_success(
                            ...
                            SmolStr::new(format!(
                                "{}{}",
                                plugin.name,
                                bentodesk_style::t(if enabled {
                                    bentodesk_style::i18n_zh_cn::ids::PLUGIN_STATUS_ENABLED_SUFFIX
                                } else {
                                    bentodesk_style::i18n_zh_cn::ids::PLUGIN_STATUS_DISABLED_SUFFIX
                                })
                            )),
                        );
                        effects.needs_redraw = true;
                    ...
                    Err(error) => {
                        set_plugin_setting_error(
                                    root,
                                    localized_plugin_message(
                                        bentodesk_style::i18n_zh_cn::ids::PLUGIN_STATUS_TOGGLE_FAILED_PREFIX,
                                        error,
                                    ),
                                );
                        effects.needs_redraw = true;
                ...
                Ok(()) => match refresh_plugin_dependent_state(root) {
                    Ok(_changed) => {
                        ...
                        set_plugin_setting_success(
                            root,
                            localized_plugin_message(
                                bentodesk_style::i18n_zh_cn::ids::PLUGIN_STATUS_REMOVED_PREFIX,
                                plugin_id,
                            ),
                        );
                        effects.needs_redraw = true;
                    ...
                    Err(error) => {
                        set_plugin_setting_error(
                                    root,
                                    localized_plugin_message(
                                        bentodesk_style::i18n_zh_cn::ids::PLUGIN_STATUS_UNINSTALL_FAILED_PREFIX,
                                        error,
                                    ),
                                );
                        effects.needs_redraw = true;
