    ...
    match validate_keybinding_candidate(root, action.as_str(), chord.as_str()) {
        Ok(()) => {
            ...
            root.dispatcher.push(Command::SetSetting {
                key,
                value: bentodesk_app::SettingValue::Str(SmolStr::new(chord)),
            });
        }
...
pub(super) fn handle_settings_text_char(root: &AppRoot, codepoint: u32) -> bool {
    ...
    if !matches!(
        ...
        bentodesk_app::SettingsTextField::DesktopPath
            | bentodesk_app::SettingsTextField::WatchValues
            | bentodesk_app::SettingsTextField::AccentColor
    ) {
        return false;
    }
...
pub(super) fn handle_settings_text_keydown(root: &AppRoot, vk: u32, hwnd: HWND) -> Option<LRESULT> {
    ...
    match vk {
        ...
        VK_ENTER => {
            let app = root.app.borrow();
            if matches!(field, bentodesk_app::SettingsTextField::WatchValues) {
