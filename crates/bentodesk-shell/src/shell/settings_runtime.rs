...
pub(super) fn apply_show_in_taskbar(hwnd: HWND, show: bool) -> Result<(), SmolStr> {
    ...
    let _guard = bentodesk_backend::ghost_layer::bypass_subclass_guard();
...
pub(super) fn apply_desktop_embed(hwnd: HWND, enabled: bool) -> Result<(), SmolStr> {
    let result = if enabled {
        ...
        bentodesk_backend::ghost_layer::detach(hwnd)
    };
    let zh = bentodesk_style::current_locale_is(&bentodesk_style::ZH_CN);
...
pub(super) fn apply_runtime_settings(
    ...
    if previous.launch_at_startup != desired.launch_at_startup {
        bentodesk_backend::autostart::set_enabled(desired.launch_at_startup).map_err(|error| {
            localized_message(
                zh,
        ...
        if bentodesk_backend::autostart::is_enabled() != desired.launch_at_startup {
            return Err(localized_message(
                zh,
                "开机启动注册表状态校验失败",
...
pub(super) fn snapshot_vault_settings(
    ...
    &'static str,
    Option<bentodesk_backend::config_vault::SettingValue>,
)> {
...
pub(super) fn persist_settings_snapshot_to_vault(
    ...
    vault.set_setting(
        ...
        bentodesk_backend::config_vault::SettingValue::Bool(snapshot.ghost_layer_enabled),
    );
    vault.set_setting(
        SETTING_GENERAL_LAUNCH_AT_STARTUP,
        bentodesk_backend::config_vault::SettingValue::Bool(snapshot.launch_at_startup),
    );
    vault.set_setting(
        SETTING_GENERAL_SHOW_IN_TASKBAR,
        bentodesk_backend::config_vault::SettingValue::Bool(snapshot.show_in_taskbar),
    );
    vault.set_setting(
        SETTING_GENERAL_AUTO_GROUP_ENABLED,
        bentodesk_backend::config_vault::SettingValue::Bool(snapshot.auto_group_enabled),
    );
    vault.set_setting(
        SETTING_GENERAL_PORTABLE_MODE,
        bentodesk_backend::config_vault::SettingValue::Bool(snapshot.portable_mode),
    );
    vault.set_setting(
        SETTING_PERF_EXPAND_DELAY_MS,
        bentodesk_backend::config_vault::SettingValue::Int(snapshot.expand_delay_ms as i64),
    );
    vault.set_setting(
        SETTING_PERF_COLLAPSE_DELAY_MS,
        bentodesk_backend::config_vault::SettingValue::Int(snapshot.collapse_delay_ms as i64),
    );
    vault.set_setting(
        SETTING_PERF_ICON_CACHE_SIZE,
        bentodesk_backend::config_vault::SettingValue::Int(snapshot.icon_cache_size as i64),
    );
    vault.set_setting(
        SETTING_STARTUP_HIGH_PRIORITY,
        bentodesk_backend::config_vault::SettingValue::Bool(snapshot.startup_high_priority),
    );
    vault.set_setting(
        SETTING_STARTUP_CRASH_RESTART_ENABLED,
        bentodesk_backend::config_vault::SettingValue::Bool(snapshot.crash_restart_enabled),
    );
    vault.set_setting(
        SETTING_STARTUP_CRASH_MAX_RETRIES,
        bentodesk_backend::config_vault::SettingValue::Int(snapshot.crash_max_retries as i64),
    );
    vault.set_setting(
        SETTING_STARTUP_CRASH_WINDOW_SECS,
        bentodesk_backend::config_vault::SettingValue::Int(snapshot.crash_window_secs as i64),
    );
    vault.set_setting(
        SETTING_STARTUP_SAFE_AFTER_HIBERNATION,
        bentodesk_backend::config_vault::SettingValue::Bool(snapshot.safe_start_after_hibernation),
    );
    vault.set_setting(
        SETTING_STARTUP_HIBERNATE_RESUME_DELAY_MS,
        bentodesk_backend::config_vault::SettingValue::Int(
    ...
    vault.set_setting(
        ...
        bentodesk_backend::config_vault::SettingValue::Str(snapshot.desktop_path_draft.clone()),
    );
    vault.set_setting(
        SETTING_PATHS_WATCH_PATHS,
        bentodesk_backend::config_vault::SettingValue::Str(snapshot.watch_paths_draft.clone()),
    );
    vault.set_setting(
        SETTING_ACTIVE_THEME,
        bentodesk_backend::config_vault::SettingValue::Str(snapshot.active_theme_id.clone()),
    );
    vault.set_setting(
        SETTING_ZONE_DISPLAY_MODE,
        bentodesk_backend::config_vault::SettingValue::Str(SmolStr::new_static(
