...
pub(super) enum SettingsBackupError {
    ...
    Vault(bentodesk_backend::config_vault::VaultError),
    Io {
        op: &'static str,
        path: PathBuf,
        source: std::io::Error,
...
impl From<bentodesk_backend::config_vault::VaultError> for SettingsBackupError {
    fn from(value: bentodesk_backend::config_vault::VaultError) -> Self {
        Self::Vault(value)
    }
}
...
pub(super) fn startup_layout_viewport(root: &AppRoot) -> bentodesk_style::Size {
    ...
    let (width, height) = bentodesk_platform::default_size(WindowKind::Main);
...
pub(super) enum RecoveryBundleShellError {
    ...
    Backend(bentodesk_backend::recovery_bundle::RecoveryBundleError),
    Vault(bentodesk_backend::config_vault::VaultError),
    Stealth(bentodesk_backend::stealth::StealthError),
    IconPosition(bentodesk_backend::icon_positions::IconPositionError),
    Platform(PlatformError),
    Io {
        op: &'static str,
        path: PathBuf,
...
impl From<bentodesk_backend::recovery_bundle::RecoveryBundleError> for RecoveryBundleShellError {
    fn from(value: bentodesk_backend::recovery_bundle::RecoveryBundleError) -> Self {
        Self::Backend(value)
    }
}

impl From<bentodesk_backend::config_vault::VaultError> for RecoveryBundleShellError {
    fn from(value: bentodesk_backend::config_vault::VaultError) -> Self {
        Self::Vault(value)
    }
}

impl From<bentodesk_backend::stealth::StealthError> for RecoveryBundleShellError {
    fn from(value: bentodesk_backend::stealth::StealthError) -> Self {
        Self::Stealth(value)
    }
}

impl From<bentodesk_backend::icon_positions::IconPositionError> for RecoveryBundleShellError {
    fn from(value: bentodesk_backend::icon_positions::IconPositionError) -> Self {
...
pub(super) enum RecoveryIconRestoreOutcome {
    NotIncluded,
    Restored(bentodesk_backend::icon_positions::RestoreResult),
    Failed(SmolStr),
}
...
pub(super) struct RecoveryBundleRestoreOutcome {
    pub(super) summary: bentodesk_backend::recovery_bundle::RecoveryBundleSummary,
    pub(super) icon_restore: RecoveryIconRestoreOutcome,
    pub(super) user_data_restore: bentodesk_backend::recovery_bundle::RecoveryUserDataRestoreReport,
}

pub(super) fn recovery_user_data_status_suffix(
    report: bentodesk_backend::recovery_bundle::RecoveryUserDataRestoreReport,
) -> SmolStr {
