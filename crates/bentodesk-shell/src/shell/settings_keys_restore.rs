...
// `desktop_path` (string) + `watch_paths` (string[]). The native vault stores
...
pub(super) fn apply_general_settings_from_vault(app: &AppState) {
    ...
    app.setting_autostart
        .set(bentodesk_backend::autostart::is_enabled());
...
pub(super) fn restore_general_str_cell(
    ...
    value: Option<bentodesk_backend::config_vault::SettingValue>,
    label: &'static str,
) {
    match value {
        Some(bentodesk_backend::config_vault::SettingValue::Str(s)) => {
            *cell.borrow_mut() = s;
        }
        Some(_) => {
            tracing::warn!(
...
pub(super) fn restore_general_int_cell(
    ...
    match value {
        Some(bentodesk_backend::config_vault::SettingValue::Int(i)) => {
...
pub(super) fn restore_general_bool_cell(
    ...
    match value {
        Some(bentodesk_backend::config_vault::SettingValue::Bool(b)) => cell.set(b),
        Some(_) => {
            tracing::warn!(
                target: "bentodesk::vault",
                key = %label,
...
pub(super) fn persist_settings_accent_to_vault(
    ...
    vault.set_setting(
        ...
        bentodesk_backend::config_vault::SettingValue::Str(accent.clone()),
    );
    vault.set_setting(
