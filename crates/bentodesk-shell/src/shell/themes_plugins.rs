...
pub(super) fn migrate_legacy_tauri_settings_to_vault(state_dir: &Path) {
    ...
    match bentodesk_backend::config_vault::tauri_settings::migrate_first_existing_tauri_settings(
        state_dir,
        &mut vault,
        &accepted_accents,
    ) {
...
pub(super) fn theme_options_from_themes(
    themes: &[bentodesk_backend::themes::Theme],
) -> Vec<ThemeOption> {
    themes
        .iter()
...
pub(super) fn load_themes_for_root(
    root: &AppRoot,
) -> Result<Vec<bentodesk_backend::themes::Theme>, ThemeError> {
...
pub(super) fn import_theme_for_root(
    root: &AppRoot,
    source_path: &Path,
) -> Result<bentodesk_backend::themes::Theme, ThemeError> {
...
pub(super) fn plugin_type_label(plugin_type: &PluginType) -> SmolStr {
    ...
    match plugin_type {
        PluginType::Theme => SmolStr::new(bentodesk_style::t(ids::PLUGIN_TYPE_THEME)),
        PluginType::Widget => SmolStr::new(bentodesk_style::t(ids::PLUGIN_TYPE_WIDGET)),
        PluginType::Organizer => SmolStr::new(bentodesk_style::t(ids::PLUGIN_TYPE_ORGANIZER)),
    }
}
...
pub(super) fn localized_plugin_message(
    prefix_id: bentodesk_style::StringId,
    detail: impl core::fmt::Display,
) -> SmolStr {
    SmolStr::new(format!("{}{detail}", bentodesk_style::t(prefix_id)))
}

pub(super) fn load_theme_selection_for_root(
    root: &AppRoot,
    id: &str,
) -> Result<(Vec<ThemeOption>, bentodesk_backend::themes::Theme), ThemeError> {
...
pub(super) fn apply_active_theme_selection_to_app(
    ...
    theme: bentodesk_backend::themes::Theme,
    ...
    app.settings_theme_status
        ...
        .replace(SettingsBackupStatus::Success(SmolStr::new(format!(
            "{} {}",
            bentodesk_style::t(bentodesk_style::i18n_zh_cn::ids::SETTINGS_THEME_HEADING),
            theme.name
        ))));
    Ok(changed)
}
...
pub(super) fn active_theme_id_is_builtin(id: &str) -> bool {
    bentodesk_style::tokens::palette_tauri_for_theme(id).is_some()
}
...
pub(super) fn persist_active_theme_to_vault(
    ...
    vault.set_setting(
        ...
        bentodesk_backend::config_vault::SettingValue::Str(id.clone()),
    );
    vault.flush()?;
    Ok(true)
}
...
pub(super) fn persist_zone_display_mode_to_vault(
    ...
    vault.set_setting(
        ...
        bentodesk_backend::config_vault::SettingValue::Str(SmolStr::new_static(mode.as_wire())),
    );
    vault.flush()?;
    Ok(true)
}
    ...
    match mtx.lock() {
        Ok(mut vault) => {
            ...
            vault.set_setting(
                ...
                bentodesk_backend::config_vault::SettingValue::Str(version.clone()),
            );
