...
pub(super) fn apply_hotkey_setting_to_runtime(
    ...
    match value {
        bentodesk_app::SettingValue::Str(chord) => apply_hotkey_binding(root, action, chord),
        _ => {
            tracing::warn!(
                target: "bentodesk::hotkey",
                %key,
...
pub(super) fn queue_locale_setting_toggle(root: &AppRoot) {
    let next_locale = if bentodesk_style::current_locale_is(&bentodesk_style::ZH_CN) {
        "en-US"
    } else {
        "zh-CN"
    };
...
pub(super) fn locale_setting_command_for(locale: &'static str) -> Command {
    Command::SetSetting {
        ...
        value: bentodesk_app::SettingValue::Str(SmolStr::new_static(locale)),
    }
}
...
pub(super) fn queue_zone_display_mode_set(root: &AppRoot, mode: bentodesk_app::ZoneDisplayMode) {
    let app = root.app.borrow();
...
pub(super) fn update_frequency_setting_command_for(frequency: UpdateCheckFrequency) -> Command {
    Command::SetSetting {
        ...
        value: bentodesk_app::SettingValue::Str(SmolStr::new_static(update_frequency_wire(
            frequency,
        ))),
    }
}
...
pub(super) fn encryption_mode_setting_command_for(mode: SettingsEncryptionMode) -> Command {
    Command::SetSetting {
        ...
        value: bentodesk_app::SettingValue::Str(SmolStr::new_static(mode.as_wire())),
    }
}
...
pub(super) fn localized_encryption_mode_button_label(mode: SettingsEncryptionMode) -> &'static str {
    ...
    match mode {
        SettingsEncryptionMode::None => bentodesk_style::t(ids::ENCRYPTION_MODE_NONE),
        SettingsEncryptionMode::Dpapi => bentodesk_style::t(ids::ENCRYPTION_MODE_DPAPI),
        SettingsEncryptionMode::Passphrase => {
            bentodesk_style::t(ids::ENCRYPTION_MODE_PASSPHRASE_FULL)
        }
    }
}
...
pub(super) fn focus_passphrase_field(app: &AppState) {
    ...
    app.settings_focused_field
        .set(bentodesk_app::SettingsTextField::Passphrase);
}
    ...
    if draft.is_empty() {
        app.settings_encryption_status
            ...
            .replace(SettingsBackupStatus::Error(SmolStr::new(
                bentodesk_style::t(bentodesk_style::i18n_zh_cn::ids::ENCRYPTION_REQUIRED),
            )));
        return None;
    }
    let purpose = if app.passphrase_unlock_required.get() {
...
pub(super) fn set_encryption_mode_applied_banner(app: &AppState) {
    ...
    let prefix = bentodesk_style::t(bentodesk_style::i18n_zh_cn::ids::ENCRYPTION_MODE_APPLIED);
...
pub(super) fn bool_setting_command_for(key: &'static str, value: bool) -> Command {
    Command::SetSetting {
        ...
        value: bentodesk_app::SettingValue::Bool(value),
    }
}
...
pub(super) fn update_check_interval(
    ...
    bentodesk_backend::updater::check_interval_hours(frequency)
...
) -> bentodesk_backend::config_vault::EncryptionMode {
    match mode {
        SettingsEncryptionMode::None => bentodesk_backend::config_vault::EncryptionMode::None,
        SettingsEncryptionMode::Dpapi => bentodesk_backend::config_vault::EncryptionMode::Dpapi,
        SettingsEncryptionMode::Passphrase => bentodesk_backend::config_vault::EncryptionMode::None,
    }
}
    ...
    match accent {
        Some(hex) => {
            ...
            vault.set_setting(
                ...
                bentodesk_backend::config_vault::SettingValue::Str(hex.clone()),
            );
        }
        None => {
