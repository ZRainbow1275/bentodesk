...
pub(super) fn load_desktop_icon_layout_for_highlight(
    root: &AppRoot,
) -> Option<bentodesk_backend::icon_positions::SavedIconLayout> {
    let state_dir = state_dir_for_root(root);
    match bentodesk_backend::icon_positions::load_from_file(&state_dir) {
    ...
    match bentodesk_backend::icon_positions::save_layout() {
