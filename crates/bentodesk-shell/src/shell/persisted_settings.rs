...
pub(super) fn write_minibar_pins_to_vault(
    ...
    match current_minibar_pins_csv(root) {
        Some(value) => vault.set_setting(
            ...
            bentodesk_backend::config_vault::SettingValue::Str(value),
        ),
        None => {
...
pub(super) fn restore_persisted_minibars_from_vault(root: &AppRoot) {
    ...
    match setting {
        Some(bentodesk_backend::config_vault::SettingValue::Str(value)) => {
...
pub(super) fn apply_persisted_settings_from_vault(root: &AppRoot) {
    ...
    let keybinding_settings = hotkey::HOTKEY_ACTIONS
        ...
        .filter_map(|action| {
            ...
            match vault.get_setting(&key) {
                Some(bentodesk_backend::config_vault::SettingValue::Str(chord)) => {
                    Some((*action, chord))
                }
                Some(_) => {
                    tracing::warn!(
    ...
    match locale_setting {
        Some(bentodesk_backend::config_vault::SettingValue::Str(locale)) => {
    ...
    match update_frequency_setting {
        Some(bentodesk_backend::config_vault::SettingValue::Str(frequency)) => {
    ...
    match update_auto_download_setting {
        Some(bentodesk_backend::config_vault::SettingValue::Bool(value)) => {
    ...
    match update_skipped_version_setting {
        Some(bentodesk_backend::config_vault::SettingValue::Str(version)) => {
    ...
    match encryption_mode_setting {
        Some(bentodesk_backend::config_vault::SettingValue::Str(mode)) => {
    ...
    match theme_base_accent_setting {
        Some(bentodesk_backend::config_vault::SettingValue::Str(accent)) => {
    ...
    match active_theme_setting {
        Some(bentodesk_backend::config_vault::SettingValue::Str(theme_id)) => {
...
pub(super) fn apply_setting_value_to_app(
    ...
    match (key, value) {
        (SETTING_UPDATES_CHECK_FREQUENCY, bentodesk_app::SettingValue::Str(frequency)) => {
        ...
        (SETTING_UPDATES_AUTO_DOWNLOAD, bentodesk_app::SettingValue::Bool(value)) => {
            let changed = app.update_auto_download.get() != *value;
            app.update_auto_download.set(*value);
            changed
        }
        (SETTING_STEALTH_ENABLED, bentodesk_app::SettingValue::Bool(value)) => {
            let changed = app.stealth_enabled.get() != *value;
            app.stealth_enabled.set(*value);
            changed
        }
        (SETTING_DEBUG_OVERLAY, bentodesk_app::SettingValue::Bool(value)) => {
            let mut overlay = app.debug_overlay.borrow_mut();
            let changed = overlay.visible != *value;
            overlay.visible = *value;
            changed
        }
        (SETTING_ENCRYPTION_MODE, bentodesk_app::SettingValue::Str(mode)) => {
        ...
        (SETTING_ZONE_DISPLAY_MODE, bentodesk_app::SettingValue::Str(mode)) => {
...
pub(super) fn persist_setting_to_vault(
    ...
    key: &str,
    value: &bentodesk_app::SettingValue,
) -> Result<bool, bentodesk_backend::config_vault::VaultError> {
    if vault.is_locked_passphrase() {
        return Err(bentodesk_backend::config_vault::VaultError::NoPassphraseSet);
    }
    let backend_value = backend_setting_value_from_app(value);
    if key == SETTING_ENCRYPTION_MODE {
        let Some(mode) = (match value {
            bentodesk_app::SettingValue::Str(mode) => encryption_mode_from_wire(mode.as_str()),
            _ => None,
        }) else {
            tracing::warn!(
                target: "bentodesk::vault",
    ...
    if key == SETTING_ZONE_DISPLAY_MODE {
        let Some(mode) = (match value {
            bentodesk_app::SettingValue::Str(mode) => zone_display_mode_from_wire(mode.as_str()),
            _ => None,
        }) else {
            tracing::warn!(
                target: "bentodesk::vault",
...
pub(super) fn persist_passphrase_to_vault(
    vault: &mut bentodesk_backend::config_vault::Vault,
    passphrase: &str,
) -> Result<(), bentodesk_backend::config_vault::VaultError> {
    vault.set_setting(
        SETTING_ENCRYPTION_MODE,
        bentodesk_backend::config_vault::SettingValue::Str(SmolStr::new_static("Passphrase")),
    );
    vault.set_mode(
        bentodesk_backend::config_vault::EncryptionMode::Passphrase {
            passphrase: SmolStr::new(passphrase),
        },
    )?;
    vault.flush()?;
    let reopened =
        bentodesk_backend::config_vault::Vault::open_with_passphrase(vault.path(), passphrase)?;
    *vault = reopened;
    Ok(())
}
