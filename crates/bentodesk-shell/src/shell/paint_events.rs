...
pub(super) unsafe fn paint(hwnd: HWND) -> Result<(), bentodesk_app::RenderError> {
    ...
    if app.settings_focused_field.get() != bentodesk_app::SettingsTextField::None {
        any_active = true;
    }
    ...
    while let Ok(event) = root.desktop_events.try_recv() {
        ...
        let mut event_changed = false;
        {
            ...
            match event.event_type.as_str() {
                ...
                "modify" => {
                    let item_path = bentodesk_app::ItemPath::new(event.path.as_str());
...
pub(super) fn drain_ghost_events(root: &AppRoot) -> bool {
    ...
    while let Ok(event) = root.ghost_events.try_recv() {
        drained = drained.saturating_add(1);
        match event {
            bentodesk_backend::ghost_layer::GhostLayerEvent::PowerResume => {
