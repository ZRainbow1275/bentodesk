...
pub(super) fn handle_settings_lbutton_down(root: &AppRoot, hwnd: HWND, x: f32, y: f32) {
    ...
    match settings_hit {
            ...
            match selected {
                ...
                Ok(None) => {
                    set_plugin_setting_success(
                        root,
                        SmolStr::new(bentodesk_style::t(
                            bentodesk_style::i18n_zh_cn::ids::PLUGIN_STATUS_INSTALL_CANCELLED,
                        )),
                    );
                    request_redraw(hwnd);
                }
                Err(error) => {
                    set_plugin_setting_error(
                        ...
                        localized_plugin_message(
                            bentodesk_style::i18n_zh_cn::ids::PLUGIN_STATUS_INSTALL_FAILED_PREFIX,
                            error,
                        ),
                    );
                    request_redraw(hwnd);
        ...
        ui::SettingsHit::SelectTheme(id) => {
            ...
            if let Some(preset) = bentodesk_app::theme_picker::BUILTIN_THEMES
                .iter()
                .find(|p| p.id == id)
            {
                    ...
                    if transition_started {
                        log_static(
                            format!(
                                ...
                                bentodesk_app::state::THEME_TRANSITION_MS
                            )
                            .as_str(),
                        );
        ...
        ui::SettingsHit::SelectAccent(index) => {
            ...
            if let Some(hex) = bentodesk_app::theme_picker::accent_swatch_hex(index as usize) {
                let app = root.app.borrow();
        ...
        ui::SettingsHit::EditDesktopPath => {
            ...
            // editing. Mutually exclusive with passphrase capture.
            {
                let app = root.app.borrow();
                app.settings_focused_field
                    .set(bentodesk_app::SettingsTextField::DesktopPath);
                app.passphrase_entry_active.set(false);
            }
            request_redraw(hwnd);
        }
        ui::SettingsHit::EditWatchValues => {
            ...
            // editing. Mutually exclusive with passphrase capture.
            {
                let app = root.app.borrow();
                app.settings_focused_field
                    .set(bentodesk_app::SettingsTextField::WatchValues);
                app.passphrase_entry_active.set(false);
            }
            request_redraw(hwnd);
        }
        ui::SettingsHit::DragPerformanceSlider { index, x_q } => {
            use bentodesk_app::settings_panel::settings_performance_slider_rect;
            use bentodesk_app::state::{
        ...
        ui::SettingsHit::IncCrashMaxRetries => {
            use bentodesk_app::state::CRASH_MAX_RETRIES_MAX;
            let app = root.app.borrow();
        ...
        ui::SettingsHit::DecCrashMaxRetries => {
            use bentodesk_app::state::CRASH_MAX_RETRIES_MIN;
            let app = root.app.borrow();
        ...
        ui::SettingsHit::IncCrashWindowSecs => {
            use bentodesk_app::state::CRASH_WINDOW_SECS_MAX;
            let app = root.app.borrow();
        ...
        ui::SettingsHit::DecCrashWindowSecs => {
            use bentodesk_app::state::CRASH_WINDOW_SECS_MIN;
            let app = root.app.borrow();
        ...
        ui::SettingsHit::DragHibernateDelay(x_q) => {
            use bentodesk_app::settings_panel::settings_hibernate_slider_rect;
            use bentodesk_app::state::{
        ...
        ui::SettingsHit::ReapplyStealth => {
            ...
            match stealth_config_now(root) {
                Some(config) => {
                    match bentodesk_backend::stealth::sync::reapply_hidden_on_startup(&config) {
                        Ok(count) => {
                            tracing::info!(
