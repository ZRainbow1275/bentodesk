...
pub(super) fn hide_item_file(
    ...
    match bentodesk_backend::stealth::hide_file(
        &config,
        source_path,
        &zone_id.0.to_string(),
...
where
    IconHashForPath: FnOnce(&bentodesk_app::ItemPath) -> Option<String>,
{
    let source = Path::new(path);
    let leaf = item_operation_leaf(path);
    if !bentodesk_backend::desktop_sources::is_under_any_desktop(source, desktop_source) {
        tracing::warn!(
            target: "bentodesk::items",
            ?zone_id,
            path,
...
pub(super) fn item_filesystem_path(item: &bentodesk_zone::ZoneItem) -> &str {
    item.hidden_path
        .as_deref()
        .unwrap_or_else(|| item.path.as_ref())
}

pub(super) fn item_file_display_path(item: &bentodesk_zone::ZoneItem) -> String {
    item.original_path
        .as_deref()
        .unwrap_or_else(|| item.path.as_ref())
        .to_owned()
