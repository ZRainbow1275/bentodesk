...
pub(super) unsafe fn show_tray_menu(root: &AppRoot, hwnd: HWND) {
    ...
    // Exit) with three dividers; native sources both the order and the labels
...
pub(super) fn show_tooltip(
    ...
    let (w, h) = bentodesk_platform::default_size(WindowKind::Tooltip);
...
pub(super) unsafe fn show_context_menu(
    root: &AppRoot,
    anchor: bentodesk_app::WindowHandle,
    items: &bentodesk_app::ContextMenuItems,
) {
    use bentodesk_app::business::popover::Popover;
