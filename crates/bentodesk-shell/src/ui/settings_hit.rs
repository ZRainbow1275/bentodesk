...
//
// Geometry constants live in `bentodesk_app::settings_panel` so the renderer
...
// `bentodesk-app::settings_panel` but no longer referenced from this hit
...
// the fully-qualified `bentodesk_app::settings_panel::settings_plugin_*` paths
// (same convention as the Backup §9 hits).
pub use bentodesk_app::settings_panel::{
...
pub enum SettingsHit {
    ...
    SetZoneDisplayMode(bentodesk_app::ZoneDisplayMode),
    ...
    // arms in `bentodesk-shell::main` continue to link.
    // ------------------------------------------------------------------
    ...
    // variants dispatch to a REAL `bentodesk_backend::stealth` call (no
...
pub fn settings_hit(app: &AppState, x: f32, y: f32) -> SettingsHit {
    ...
    if app.settings_keybindings_open.get() {
        ...
        if x >= modal.x && x < modal.right() && y >= modal.y && y < modal.bottom() {
            ...
            for row_index in
                0..bentodesk_app::business::settings::keybindings_section::keybinding_rows().len()
            {
    ...
    let panel = bentodesk_app::settings_panel::settings_panel_rect_m1(vp);
    ...
    let close_m1 = bentodesk_app::settings_panel::settings_close_button_rect_m1(vp);
    if x >= close_m1.x && x < close_m1.right() && y >= close_m1.y && y < close_m1.bottom() {
        return SettingsHit::Close;
    }
    // Footer Cancel + Save — sticky.
    let cancel_btn = bentodesk_app::settings_panel::settings_cancel_button_rect(vp);
    if x >= cancel_btn.x && x < cancel_btn.right() && y >= cancel_btn.y && y < cancel_btn.bottom() {
        return SettingsHit::CancelSettings;
    }
    let save_btn = bentodesk_app::settings_panel::settings_save_button_rect(vp);
    ...
    // surfaces.
    let body = bentodesk_app::settings_panel::settings_body_rect(vp);
    if x < body.x || x >= body.right() || y < body.y || y >= body.bottom() {
        return SettingsHit::Body;
    }
    let scroll_y = app.scroll_offset_y.get();
    // Top 5 toggles — index 0..=4.
    for index in 0..bentodesk_app::settings_panel::SETTINGS_TOP_TOGGLE_COUNT {
        let hit = bentodesk_app::settings_panel::settings_top_toggle_hit_rect(vp, scroll_y, index);
    ...
    let lang_chip = bentodesk_app::settings_panel::settings_language_chip_rect(vp, scroll_y);
    ...
    let refresh = bentodesk_app::settings_panel::settings_sources_refresh_button_rect(
        vp,
        scroll_y,
        source_count,
    );
    ...
    let path_box =
        bentodesk_app::settings_panel::settings_desktop_path_input_rect(vp, scroll_y, source_count);
    ...
    let watch_box =
        bentodesk_app::settings_panel::settings_watch_textarea_rect(vp, scroll_y, source_count);
    ...
    let scroll_y =
        scroll_y + bentodesk_app::settings_panel::settings_sources_reserve_delta(source_count);
    ...
    for index in 0..bentodesk_app::settings_panel::SETTINGS_ZONE_DISPLAY_MODE_COUNT {
        let hit = bentodesk_app::settings_panel::settings_zone_display_mode_radio_rect(
            vp, scroll_y, index,
        );
        if x >= hit.x && x < hit.right() && y >= hit.y && y < hit.bottom() {
            let mode = match index {
                0 => bentodesk_app::ZoneDisplayMode::Hover,
                1 => bentodesk_app::ZoneDisplayMode::Always,
                2 => bentodesk_app::ZoneDisplayMode::Click,
    ...
    // track-x→value map.
    for index in 0..bentodesk_app::settings_panel::SETTINGS_PERF_ROW_COUNT {
        let track =
            bentodesk_app::settings_panel::settings_performance_slider_rect(vp, scroll_y, index);
    ...
    // 高优先级启动 toggle (row 0).
    let high_row =
        bentodesk_app::settings_panel::settings_startup_high_priority_row_rect(vp, scroll_y);
    let high_hit = bentodesk_app::settings_panel::settings_startup_toggle_hit_rect(high_row);
    if x >= high_hit.x && x < high_hit.right() && y >= high_hit.y && y < high_hit.bottom() {
        return SettingsHit::ToggleStartupHighPriority;
    }
    // 崩溃自动重启 toggle (row 1).
    let crash_row = bentodesk_app::settings_panel::settings_crash_restart_row_rect(vp, scroll_y);
    let crash_hit = bentodesk_app::settings_panel::settings_startup_toggle_hit_rect(crash_row);
    ...
    if crash_restart_on {
        let retries_row =
            bentodesk_app::settings_panel::settings_crash_max_retries_row_rect(vp, scroll_y);
        let r_plus = bentodesk_app::settings_panel::settings_stepper_plus_rect(retries_row);
        if x >= r_plus.x && x < r_plus.right() && y >= r_plus.y && y < r_plus.bottom() {
            return SettingsHit::IncCrashMaxRetries;
        }
        let r_minus = bentodesk_app::settings_panel::settings_stepper_minus_rect(retries_row);
        if x >= r_minus.x && x < r_minus.right() && y >= r_minus.y && y < r_minus.bottom() {
            return SettingsHit::DecCrashMaxRetries;
        }
        let window_row =
            bentodesk_app::settings_panel::settings_crash_window_row_rect(vp, scroll_y);
        let w_plus = bentodesk_app::settings_panel::settings_stepper_plus_rect(window_row);
        if x >= w_plus.x && x < w_plus.right() && y >= w_plus.y && y < w_plus.bottom() {
            return SettingsHit::IncCrashWindowSecs;
        }
        let w_minus = bentodesk_app::settings_panel::settings_stepper_minus_rect(window_row);
    ...
    let safe_row =
        bentodesk_app::settings_panel::settings_safe_start_row_rect(vp, scroll_y, crash_restart_on);
    let safe_hit = bentodesk_app::settings_panel::settings_startup_toggle_hit_rect(safe_row);
    ...
    if safe_start_on {
        let track = bentodesk_app::settings_panel::settings_hibernate_slider_rect(
            vp,
            scroll_y,
            crash_restart_on,
        );
    ...
    let stealth_btn_row = bentodesk_app::settings_panel::settings_stealth_buttons_row_rect(
        vp,
        scroll_y,
        crash_restart_on,
        safe_start_on,
    ...
    let refresh_btn =
        bentodesk_app::settings_panel::settings_stealth_refresh_button_rect(stealth_btn_row);
    if x >= refresh_btn.x
        && x < refresh_btn.right()
        && y >= refresh_btn.y
    ...
    let reapply_btn =
        bentodesk_app::settings_panel::settings_stealth_reapply_button_rect(stealth_btn_row);
    if x >= reapply_btn.x
        && x < reapply_btn.right()
        && y >= reapply_btn.y
    ...
    let updater_kind =
        bentodesk_app::business::settings::updater_card::updater_height_kind(&updater_status);
    let updater_flags = bentodesk_app::settings_panel::SettingsBodyFlags::new(
        crash_restart_on,
        safe_start_on,
        stealth_has_retry,
        stealth_has_error,
    ...
    let upd_btn_row = bentodesk_app::settings_panel::settings_updater_buttons_row_rect(
        vp,
        scroll_y,
        &updater_flags,
    );
    // Col 0 — 检查更新 (always).
    let check_btn = bentodesk_app::settings_panel::settings_updater_button_rect(upd_btn_row, 0);
    ...
    if bentodesk_app::business::settings::updater_card::updater_show_download(&updater_status)
        || bentodesk_app::business::settings::updater_card::updater_show_install(&updater_status)
    {
        let action_btn =
            bentodesk_app::settings_panel::settings_updater_button_rect(upd_btn_row, 1);
        if x >= action_btn.x
            && x < action_btn.right()
            && y >= action_btn.y
            && y < action_btn.bottom()
    ...
    if bentodesk_app::business::settings::updater_card::updater_show_skip(&updater_status) {
        let skip_btn = bentodesk_app::settings_panel::settings_updater_button_rect(upd_btn_row, 2);
    ...
    let upd_freq_row = bentodesk_app::settings_panel::settings_updater_frequency_row_rect(
        vp,
        scroll_y,
        &updater_flags,
    );
    let freq_chip =
        bentodesk_app::settings_panel::settings_updater_frequency_chip_rect(upd_freq_row);
    ...
    let upd_auto_row = bentodesk_app::settings_panel::settings_updater_auto_download_row_rect(
        vp,
        scroll_y,
        &updater_flags,
    );
    let auto_hit =
        bentodesk_app::settings_panel::settings_updater_auto_download_hit_rect(upd_auto_row);
    ...
    let backup_visible =
        bentodesk_app::business::settings::backup_card::backup_visible_row_count(&backup_entries);
    ...
    let backup_actions = bentodesk_app::settings_panel::settings_backup_actions_row_rect(
        vp,
        scroll_y,
        &backup_flags,
    );
    let create_btn =
        bentodesk_app::settings_panel::settings_backup_create_button_rect(backup_actions);
    ...
    if !bentodesk_app::business::settings::backup_card::backup_list_is_empty(&backup_entries) {
        for entry_index in 0..backup_visible {
            let entry_row = bentodesk_app::settings_panel::settings_backup_entry_row_rect(
                vp,
                scroll_y,
                &backup_flags,
                entry_index,
            ...
            let restore_btn =
                bentodesk_app::settings_panel::settings_backup_restore_button_rect(entry_row);
            if x >= restore_btn.x
                && x < restore_btn.right()
                && y >= restore_btn.y
    ...
    for index in 0..bentodesk_app::settings_panel::SETTINGS_ENCRYPTION_MODE_COUNT {
    ...
    let enc_input = bentodesk_app::settings_panel::settings_encryption_passphrase_input_rect(
        vp,
        scroll_y,
        &backup_flags,
    );
    ...
    let plugin_visible =
        bentodesk_app::business::settings::plugins_section::plugin_visible_row_count(
            &plugin_entries,
        );
    let plugin_flags = backup_flags
        .with_plugin_rows(plugin_visible)
        .with_plugin_status(app.settings_plugin_status.borrow().is_some());
    let plugin_install = bentodesk_app::settings_panel::settings_plugins_install_button_rect(
        vp,
        scroll_y,
        &plugin_flags,
    );
    ...
    if !bentodesk_app::business::settings::plugins_section::plugin_list_is_empty(&plugin_entries) {
        for card_index in 0..plugin_visible {
            ...
            let uninstall =
                bentodesk_app::settings_panel::settings_plugin_uninstall_button_rect(card);
            if app.settings_plugin_uninstall_confirm.get() == Some(card_index) {
                let cancel =
                    bentodesk_app::settings_panel::settings_plugin_uninstall_cancel_button_rect(
                        card,
                    );
    ...
    let appearance_origin =
        bentodesk_app::settings_panel::settings_appearance_grid_origin(vp, scroll_y, &plugin_flags);
    let appearance_inner_w = bentodesk_app::settings_panel::settings_appearance_inner_width(vp);
    let appearance =
        bentodesk_app::theme_picker::appearance_layout(appearance_origin, appearance_inner_w);
    match bentodesk_app::theme_picker::appearance_hit_test(&appearance, x, y) {
