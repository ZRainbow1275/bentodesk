use super::*;
use bentodesk_app::{AppState, SettingsPluginEntry};
...
fn scroll_offset_to_top_of_body(
    ...
    flags: &bentodesk_app::settings_panel::SettingsBodyFlags,
    label_at_unscrolled_y: f32,
) -> f32 {
    let body = bentodesk_app::settings_panel::settings_body_rect(viewport);
    ...
    let content_h = bentodesk_app::settings_panel::settings_body_content_height(viewport, flags);
    let max_scroll = bentodesk_app::settings_panel::settings_body_max_scroll(content_h, viewport);
    want.min(max_scroll)
}
