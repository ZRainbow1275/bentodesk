...
use bentodesk_style::{Size, ZH_CN};
use bentodesk_widget::{WidgetKind, WidgetNode};
...
fn mount_main_tree_produces_well_formed_tree() {
    ...
    let mut visited: smallvec::SmallVec<[bentodesk_tree::NodeId; 16]> = smallvec::SmallVec::new();
    visited.push(root);
    let mut i = 0;
    while i < visited.len() {
        let id = visited[i];
...
fn icon_button_paths_use_only_supported_svg_commands() {
    // The hand-rolled `bentodesk-platform::svg` parser accepts only
...
fn appstate_alloc_zone_id_is_monotonic_starting_at_one() {
    ...
    assert_eq!(a, bentodesk_zone::ZoneId(1));
    assert_eq!(b, bentodesk_zone::ZoneId(2));
    assert_eq!(c, bentodesk_zone::ZoneId(3));
}
...
// `bentodesk-style::i18n_{zh,en}` and continues to resolve correctly when
