...
//!   `AppState.layout`. Per master plan §2 the nano build is single-process
...
//!   caller (`bento-nano-app::backend_bridge`) owns settings/layout state.
...
/// nano port hoists them into one struct so callers can decide once where
...
/// nano's workspace serde dep is `default-features = false` per spec §6
