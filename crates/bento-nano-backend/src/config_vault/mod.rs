...
//! - Atomic write + `.bak` rotation via `bento-nano-backend::storage::write_json_atomic`.
//!
//! ## Public API surface
//!
//! ```ignore
...
/// Tagged setting value. Mirrors `bento-nano-app::dispatcher::SettingValue`
...
/// `lib.rs §1` — `bento-nano-backend` is layer 2.5, app is layer 3).
...
#[derive(Debug)]
pub enum VaultError {
    /// `bento-nano-backend::storage` returned an error.
...
mod tests {
    ...
    fn tempdir() -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("bentonano-vault-{pid}-{n}"));
