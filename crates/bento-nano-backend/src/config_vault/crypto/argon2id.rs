...
struct Blake2b {
    ...
    buf: [u8; 128],
    buflen: usize,
    // F3: read by `fn blake2b()` test helper and any future caller that needs
    // to truncate the digest at `finalize()`. Kept on the struct so the
    // BLAKE2b state is self-describing and matches the RFC 7693 parameter
    // block layout. See project_bentodesk_nano_2026_05_03_f1_f7_rulings.md
    // Q2=C ruling — Argon2id scaffolding lands in F1, gets wired in F3.
    #[allow(dead_code)]
    out_len: usize,
}
...
// F3: wired in by Vault::set_mode(Passphrase) — see
// project_bentodesk_nano_2026_05_03_f1_f7_rulings.md Q2=C ruling.
// The standalone BLAKE2b convenience wrapper currently only feeds the RFC 7693
// vector tests; production callers route through `h_prime` directly. Kept
// available so future KDF tuning (e.g. checksum diagnostics) does not need to
// re-derive the wrapper from scratch.
#[allow(dead_code)]
...
impl Block {
    ...
    // F3: wired in by Vault::set_mode(Passphrase) — see
    // project_bentodesk_nano_2026_05_03_f1_f7_rulings.md Q2=C ruling.
    // Reserved for the upcoming parallel-lane fan-out path where one lane's
    // final block is mirrored into another lane's working buffer. Today the
    // straight-line variant uses direct indexing, so this helper is unused.
    #[allow(dead_code)]
