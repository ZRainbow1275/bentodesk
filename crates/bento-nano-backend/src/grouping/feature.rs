//! T-087 — feature vectors + similarity functions used by the AI recommender.
//!
...
//!
//! Differences vs 1.x: `chrono::DateTime::parse_from_rfc3339(iso).timestamp()`
//! is replaced by [`crate::time::parse_rfc3339_to_unix_secs`] (Q1 ruling).
...
/// Whole-day Unix epoch for the given RFC3339 timestamp. Returns 0 on
/// parse failure (1.x parity — empty/garbage timestamps are silently
/// treated as the epoch, which the similarity decay then renders into a
/// large `day_diff`).
fn created_day(iso: &str) -> i64 {
    time::parse_rfc3339_to_unix_secs(iso)
        .map(|s| s / 86_400)
        .unwrap_or(0)
}
...
pub fn similarity(a: &FeatureVector, b: &FeatureVector) -> f64 {
    ...
    let time_decay = (-day_diff / 7.0).exp(); // decays over a week
    0.55 * name_j + 0.3 * ext + 0.15 * time_decay
}
...
mod tests {
    ...
    #[test]
    fn created_day_parses_rfc3339_through_time_helper() {
        let day = created_day("2026-01-01T00:00:00Z");
        assert!(day > 20_000, "expected > 20k days since epoch, got {day}");
    }

    #[test]
    fn created_day_returns_zero_on_garbage() {
        assert_eq!(created_day("not a date"), 0);
        assert_eq!(created_day(""), 0);
    }
}
