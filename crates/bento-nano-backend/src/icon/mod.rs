...
//!   webview is gone in nano (single-process Direct2D paint); the
...
///
/// In 1.x these values lived on `AppState.app_data_dir`; the nano port
/// hoists them so callers (typically `bento-nano-app::dispatcher`)
