...
use bento_nano_backend::icon_positions::SavedIconLayout;
use bento_nano_backend::recovery_bundle;
    ...
    I: Iterator<Item = String>,
{
    let Some(path) = args.next() else {
        return Err(
            "usage: cargo run -p bento-nano-backend --example seed_recovery_bundle -- <state-dir-or-zones.bin> <zone-count> [icon-layout-backup-json]"
                .to_owned(),
        );
    };
