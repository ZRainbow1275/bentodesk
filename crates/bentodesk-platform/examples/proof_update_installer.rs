...
fn append_marker() -> std::io::Result<Option<PathBuf>> {
    let Some(marker) = env::var_os("BENTODESK_UPDATE_PROOF_MARKER") else {
        return Ok(None);
    };
...
fn restart_shell() -> std::io::Result<()> {
    let Some(exe) = env::var_os("BENTODESK_UPDATE_PROOF_RESTART_EXE") else {
        return Ok(());
    };
    let mut command = Command::new(exe);
    if let Some(state_dir) = env::var_os("BENTODESK_STATE_DIR") {
        command.env("BENTODESK_STATE_DIR", state_dir);
    }
    command.env("BENTODESK_UPDATE_PROOF_RESTARTED", "1");
    command
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
