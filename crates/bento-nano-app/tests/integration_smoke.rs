...
//!
//! Verifies that `bento-nano-tree` + `bento-nano-layout` + `bento-nano-widget`
...
use bento_nano_widget::{ContainerNode, TextNode, WidgetNode};
...
fn smoke_tree_and_layout_compose() {
    ...
    let root = app.mount_root(WidgetNode::Container(ContainerNode {
        ...
        shadow: bento_nano_style::Shadow::NONE,
    }));
    ...
    let card_id = app
        ...
        .ok()
        .unwrap_or(bento_nano_tree::NodeId::ROOT_INVALID);
