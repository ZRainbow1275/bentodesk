...
- **Visible-glyph large left slot** (V21-C21): the source CSS padding table
  ...
  visible icon/title run 7 DIPs left of the CSS 28 px slot. Nano therefore uses
...
- **Visible-glyph large inner gap** (V21-C22): the source CSS gap table remains
  ...
  Nano therefore uses an 11 px runtime gap only for large capsules with a
...
- **Visible-glyph large content y-offset** (V21-C24): after C23, the Browser
  ...
  component crop. Nano applies a 2 DIP downshift only to Large capsules with a
...
- **No-glyph large content y-offset** (V21-C26): after C25, the right-rail
  ...
  2026-06-02 component crop at 1.5x proof scale. Nano applies a 2 DIP upshift
...
- **Large badge right inset** (V21-C27): the source CSS right padding table
  ...
  Large count badges slightly right of the 2026-06-02 crop. Nano therefore uses
...
- **Visible-glyph large badge height** (V21-C28): after C27, the Browser
  ...
  matched the reference height. Nano therefore uses the 16 px medium visual span
...
- **Visible-glyph large title font** (V21-C25): after C24, the Browser
  ...
  tier. Nano therefore resolves the Large visible-glyph title font separately
...
- **No-glyph large title/badge residual x alignment** (V21-C30): after C29, the
  ...
  pixel right of the component reference. Nano therefore uses a 0.2 px DWrite
...
- **Per-tier badge** (`:34-43/:93-96/:113-116`, C18/C23 adjusted by
  ...
  and the medium chip bbox in the authoritative recording. Nano therefore keeps
...
- **Icon box runtime contract** (V21-C4): `ZenCapsule.tsx` passes fixed
  ...
  rules remain on the outer span only, so nano keeps every collapsed/circle
  icon box at 18 px.
