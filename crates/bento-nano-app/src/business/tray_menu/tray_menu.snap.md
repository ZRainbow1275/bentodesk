...
NOTE: 1.x uses Tauri's OS-native `Menu`/`MenuItem` (`HMENU` via Win32 `TrackPopupMenu`); nano's single-process invariant means we render this menu in our own DComp visual tree as a `WindowKind::ContextMenu` HWND popup positioned near the tray icon. The TrayIcon registration (`Shell_NotifyIconW`) lives in `bento-nano-shell`; this snap.md describes the popup surface only.
...
- **i18n:** locale-aware text (Chinese in 1.x; nano keeps both via `bento-nano-style::i18n` table).
