...
//! in `bento-nano-app::Renderer::release_swap_chain` /
...
use bento_nano_widget::IconButton;
...
// `bento-nano-app::Renderer`.
// -----------------------------------------------------------------------------

/// Per-MiniBar-window hibernation contract. Implemented by
/// `bento-nano-app::Renderer` (the production wiring) and by
...
impl MiniBar {
    ...
    /// compile-time SVG path literals (see `bento-nano-app::business::icons`
    /// once T-079 lands).
    pub fn new(
