...
- **Behaviour:** `onMount` calls `listBackups()` and registers `onBackupCreated` listener. `onCreate` → `createBackup()` then refresh. `onRestore(entry)` → `confirm()` modal → `restoreBackup(entry.id)` → `loadSettings()` → refresh. Backend dep: `bento-nano-backend::config_vault::{list_backups, create_backup, restore_backup}` (T-092). Eventual dispatcher hook: emits `Command::SetSetting { key: "backup.last_restored", value: SettingValue::Str(entry_id) }` post-restore so the SettingsPanel host re-derives `dirty`.
