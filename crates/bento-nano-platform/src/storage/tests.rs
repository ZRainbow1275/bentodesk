...
#[test]
fn bentodesk_state_dir_precedes_the_pre_2_0_compatibility_alias() {
    let current = std::ffi::OsStr::new(r"C:\Temp\bentodesk-2-state");
    let legacy = std::ffi::OsStr::new(r"C:\Temp\bentodesk-nano-state");
    assert_eq!(
        state_dir_override_path_from_values(Some(current), Some(legacy)),
        Some(PathBuf::from(current).join("zones.bin"))
    );
    assert_eq!(
        state_dir_override_path_from_values(None, Some(legacy)),
        Some(PathBuf::from(legacy).join("zones.bin"))
    );
}
