...
/// instead. The pre-2.0 `BENTODESK_NANO_STATE_DIR` name remains a lower-priority
/// compatibility alias. Both are diagnostic/runtime-proof overrides so hand
/// tests can avoid mutating the user's real BentoDesk data.
///
...
fn state_dir_override_path() -> Option<PathBuf> {
    let current = std::env::var_os(STATE_DIR_ENV);
    let legacy = std::env::var_os(LEGACY_STATE_DIR_ENV);
    state_dir_override_path_from_values(current.as_deref(), legacy.as_deref())
}
...
pub(super) fn state_dir_override_path_from_values(
    current: Option<&OsStr>,
    legacy: Option<&OsStr>,
) -> Option<PathBuf> {
    current
        .and_then(state_dir_override_path_from_value)
        .or_else(|| legacy.and_then(state_dir_override_path_from_value))
}
