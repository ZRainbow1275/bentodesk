...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("zone_id\ttitle\tlive_folder_path\titem_count");
    for zone in zones.iter() {
        println!(
            "{}\t{}\t{}\t{}",
            zone.id.0,
            tsv_escape(zone.title.as_ref()),
            tsv_escape(zone.live_folder_path.as_deref().unwrap_or_default()),
            zone.items.len()
        );
    }
    Ok(())
    ...
    I: Iterator<Item = String>,
{
    let Some(first) = args.next() else {
        return Err(
            "usage: cargo run -p bento-nano-platform --example dump_zone_summary -- <state-dir-or-zones.bin>"
                .to_owned(),
        );
    };
