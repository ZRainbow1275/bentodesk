...
//! `bentodesk_style::tokens::palette_tauri_for_theme(id)`. Custom JSON
...
//!
//! `PaletteTokens` lives in `bentodesk-theme`; `PaletteTauri` lives in
//! `bentodesk-style`. The style crate is a leaf and must NOT depend on
//! `bentodesk-theme`, so the bridge cannot live there. The app crate
...
use bentodesk_theme::{PaletteTokens, RadiusTokens, ShadowTokens, ThemeTokens, radius, shadow};
...
pub fn resolve_palette_tauri(theme_id: &str, tokens: &PaletteTokens) -> PaletteTauri {
    bentodesk_style::tokens::palette_tauri_for_theme(theme_id)
...
pub fn shadow_tokens_from_tauri(st: ShadowTauri) -> ShadowTokens {
    if st.zen.is_empty() {
        ...
        ShadowTokens {
            sm: bentodesk_style::Shadow::NONE,
            md: bentodesk_style::Shadow::NONE,
            lg: bentodesk_style::Shadow::NONE,
        }
    } else {
...
pub fn theme_tokens_for_theme(id: &str, base: &ThemeTokens) -> ThemeTokens {
    ...
    if let Some(rt) = bentodesk_style::tokens::radius_tauri_for_theme(id) {
        t.radius = radius_tokens_from_tauri(rt);
    }
    if let Some(st) = bentodesk_style::tokens::shadow_tauri_for_theme(id) {
        t.shadow = shadow_tokens_from_tauri(st);
    }
    if let Some(tt) = bentodesk_style::tokens::typography_tauri_for_theme(id) {
...
mod tests {
    use super::*;
    use bentodesk_theme::{DARK_DEFAULT, LIGHT_DEFAULT};
    ...
    fn builtin_ocean_blue_resolves_to_exact_const() {
        ...
        assert_eq!(p, bentodesk_style::tokens::PALETTE_OCEAN_BLUE,);
    }
    ...
    fn radius_dark_maps_byte_identical_to_default() {
        ...
        let rt = bentodesk_style::tokens::radius_tauri_for_theme("dark").unwrap();
    ...
    fn radius_flat_collapses_all_stops_toward_4() {
        ...
        let rt = bentodesk_style::tokens::radius_tauri_for_theme("flat").unwrap();
    ...
    fn radius_brutalism_collapses_to_zero() {
        let rt = bentodesk_style::tokens::radius_tauri_for_theme("brutalism").unwrap();
        let r = radius_tokens_from_tauri(rt);
    ...
    fn radius_scale_stays_monotonic() {
        ...
        for id in ["dark", "order", "flat", "neo", "terminal", "cyberpunk"] {
            let rt = bentodesk_style::tokens::radius_tauri_for_theme(id).unwrap();
            let r = radius_tokens_from_tauri(rt);
    ...
    fn shadow_drop_themes_keep_default_byte_for_byte() {
        ...
        for id in ["dark", "light", "midnight", "ocean-blue", "solid"] {
            let st = bentodesk_style::tokens::shadow_tauri_for_theme(id).unwrap();
            assert_eq!(
    ...
    fn shadow_angular_none_themes_flatten_widget_chrome() {
        for id in ["flat", "brutalism", "editorial"] {
            ...
            assert_eq!(s.md, bentodesk_style::Shadow::NONE, "{id} shadow not flat");
        }
    }
    ...
    fn theme_tokens_editorial_uses_georgia_and_sharp_corners() {
        ...
        assert_eq!(t.shadow.md, bentodesk_style::Shadow::NONE); // no drop
    }
