...
impl Renderer {
    ...
    pub(super) fn draw_settings_panel(&mut self, app: &AppState) -> Result<(), RenderError> {
        ...
        let panel_radius = bentodesk_style::BorderRadius::all(SETTINGS_PANEL_RADIUS);
        // M6b — per-theme card radius for the Settings chip surfaces.
        let chip_radius_tokens = app.active_theme_radius_tauri();
        let chip_radius = bentodesk_style::BorderRadius::all(chip_radius_tokens.card);
        let btn_radius = bentodesk_style::BorderRadius::all(8.0);
        ...
        if !settings_panel_fills_host(viewport) {
            let scrim_rect = bentodesk_style::Rect {
                x: 0.0,
                y: 0.0,
            ...
            self.fill_rounded_rect(
                scrim_rect,
                with_alpha(bentodesk_style::Color::BLACK, 0.50),
        ...
        let settings_paint = (|| -> Result<(), RenderError> {
            ...
            let panel_border = bentodesk_style::Rect {
                x: panel.x + 0.5,
                y: panel.y + 0.5,
            ...
            self.stroke_rounded_rect(
                ...
                bentodesk_style::BorderRadius::all((SETTINGS_PANEL_RADIUS - 0.5).max(0.0)),
                1.0,
            )?;
            ...
            self.draw_text_no_wrap_with_style(
                bentodesk_style::t(bentodesk_style::i18n_zh_cn::ids::SETTINGS_TITLE),
                title_rect,
                title_color,
                16.0,
                600,
            ...
            let header_hairline = bentodesk_style::Rect {
                x: header.x,
                y: header.bottom() - 1.0,
                width: header.width,
                height: 1.0,
            ...
            let footer_hairline = bentodesk_style::Rect {
                x: footer.x,
                y: footer.y,
                width: footer.width,
                height: 1.0,
            ...
            self.draw_settings_button_text(
                bentodesk_style::t(bentodesk_style::i18n_zh_cn::ids::SETTING_SAVE),
                save_btn,
                save_text,
                13.0,
                500,
