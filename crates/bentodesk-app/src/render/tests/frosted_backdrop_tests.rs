...
use bentodesk_style::{BorderRadius, Color, Rect, Shadow, ShadowStack};
...
fn shared_frosted_backdrop_tracks_tauri_zen_blur_under_memory_budget() {
    ...
    assert!(
        (frosted_backdrop_saturation_for_palette(bentodesk_style::tokens::PALETTE_DARK) - 1.6)
            .abs()
            < f32::EPSILON
    );
    assert!(
        (frosted_backdrop_saturation_for_palette(bentodesk_style::tokens::PALETTE_LIGHT) - 1.3)
            .abs()
            < f32::EPSILON
    );
...
fn degraded_frosted_surface_stays_dense_but_translucent() {
    let zen = bentodesk_style::tokens::PALETTE_DARK.surface_zen;
...
fn lerp_color_endpoints_and_midpoint() {
    ...
    assert_eq!(at1, bentodesk_style::tokens::PALETTE_DARK.surface_expanded);
...
fn collapsed_zen_surface_ignores_hover_to_match_tauri_css() {
    let idle = collapsed_zen_surface_color(bentodesk_style::tokens::PALETTE_DARK, 0.0);
    let hover = collapsed_zen_surface_color(bentodesk_style::tokens::PALETTE_DARK, 1.0);
    let overshoot = collapsed_zen_surface_color(bentodesk_style::tokens::PALETTE_DARK, 2.0);
    assert_eq!(idle, bentodesk_style::tokens::PALETTE_DARK.surface_zen);
...
fn ordinary_pill_shadow_attenuates_medium_and_large_without_changing_geometry() {
    let idle = bentodesk_style::tokens::SHADOW.zen;
...
fn stack_capsule_locked_chrome_matches_tauri_css() {
    ...
    let pal = bentodesk_style::tokens::PALETTE_DARK;
...
fn stack_capsule_hover_border_reaches_tauri_literal_white_alpha() {
    let idle = stack_capsule_hover_border_color(bentodesk_style::tokens::PALETTE_DARK, 0.0);
    let mid = stack_capsule_hover_border_color(bentodesk_style::tokens::PALETTE_DARK, 0.5);
    let hover = stack_capsule_hover_border_color(bentodesk_style::tokens::PALETTE_DARK, 1.0);

    assert_eq!(idle, bentodesk_style::tokens::PALETTE_DARK.border_zen);
...
fn stack_capsule_hover_shadow_reaches_tauri_hover_box_shadow() {
    let idle_shadow = bentodesk_style::tokens::SHADOW.zen;
    assert_eq!(
...
fn stack_capsule_bloom_shadow_and_color_apply_css_opacity() {
    ...
    let bloomed = stack_capsule_bloom_shadow_stack(bentodesk_style::ShadowStack::NONE, 0.0, 1.0);
