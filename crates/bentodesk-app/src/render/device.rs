...
impl Renderer {
    pub fn create(hwnd: W_HWND, width: u32, height: u32) -> Result<Self, RenderError> {
        ...
        Ok(Self {
            ...
            hwnd,
            device_gen: bentodesk_platform::device_generation(),
    ...
    pub fn rebuild_after_device_loss(&mut self) -> Result<(), RenderError> {
        ...
        self.device_gen = bentodesk_platform::device_generation();
        Ok(())
    }
    ...
    pub fn render(
        ...
        if renderer_is_stale(self.device_gen, bentodesk_platform::device_generation()) {
        ...
        let device_size = bentodesk_style::Size {
        ...
        if rendered_aux_window {
            ...
            // a per-document `position:fixed; inset:0` overlay, so each native
        ...
        let mut ids: SmallVec<[(bentodesk_tree::NodeId, bentodesk_style::Rect); 32]> =
            SmallVec::new();
        {
    ...
    /// truth, mirroring `bentodesk-shell::ui::main_nchittest_kind`), are
    ...
    pub(super) fn apply_main_click_through_region(&mut self, app: &AppState) {
        ...
        // `bentodesk-platform::window::to_windows_hwnd`, the inverse bridge).
