...
impl Renderer {
    pub(super) fn draw_settings_appearance(
        ...
        // (no native keyboard/text-input infra + no JSON theme parser) — see the
        ...
        if row_visible(appearance_label, body) {
            self.draw_settings_group_title(
                bentodesk_style::t(bentodesk_style::i18n_zh_cn::ids::SETTINGS_GROUP_APPEARANCE),
                appearance_label,
                label_color,
            )?;
        }
        ...
        if row_visible(picker_label, body) {
            self.draw_settings_text(
                bentodesk_style::t(bentodesk_style::i18n_zh_cn::ids::THEME_PICKER_LABEL),
                picker_label,
                label_color,
            )?;
        }
        ...
        let card_radius = bentodesk_style::BorderRadius::all(THEME_CARD_RADIUS);
        let swatch_radius = bentodesk_style::BorderRadius::all(SWATCH_BLOCK_RADIUS);
        ...
        for (group_pos, group) in THEME_GROUP_ORDER.iter().enumerate() {
            ...
            if row_visible(heading, body) {
                self.draw_text_tracked(
                    bentodesk_style::t(group.heading_id()),
                    heading,
                    palette.text_muted,
                    10.0,
                    600,
        ...
        for preset in BUILTIN_THEMES.iter() {
            ...
            if let Some(border_color) = card_chrome.border {
                ...
                let border_rect = bentodesk_style::Rect {
                    x: card.x + inset,
                    y: card.y + inset,
                ...
                let border_radius =
                    bentodesk_style::BorderRadius::all((THEME_CARD_RADIUS - inset).max(0.0));
            ...
            self.draw_text_no_wrap_with_style(
                bentodesk_style::t(preset.name_id),
                label_rect,
        ...
        // Reuse the existing native ChooseColorW producer.
        if row_visible(appearance.accent_row, body) {
            ...
            self.draw_settings_text_no_wrap(
                bentodesk_style::t(bentodesk_style::i18n_zh_cn::ids::SETTINGS_ACCENT_COLOR),
                accent_label_rect,
                label_color,
            )?;
            let accent_picker_hovered = appearance_hover == Some(tp::AppearanceHit::AccentPicker);
            let accent_picker_radius = bentodesk_style::BorderRadius::all(6.0);
            ...
            let preview = bentodesk_style::Rect {
            ...
            self.fill_rounded_rect(
                preview,
                preview_color,
                bentodesk_style::BorderRadius::all(4.0),
            )?;
        }
        ...
        if row_visible(display_mode_label, body) {
            self.draw_settings_group_title(
                bentodesk_style::t(bentodesk_style::i18n_zh_cn::ids::SETTINGS_GROUP_DISPLAY_MODE),
                display_mode_label,
                palette.text_muted,
            )?;
        }
        ...
        if row_visible(picker_row, body) {
            ...
            self.draw_settings_text(
                bentodesk_style::t(bentodesk_style::i18n_zh_cn::ids::SETTINGS_DISPLAY_MODE_LABEL),
                copy_label,
                label_color,
            )?;
            let hint = settings_display_mode_hint_rect(viewport, scroll);
            self.draw_text_with_style(
                bentodesk_style::t(bentodesk_style::i18n_zh_cn::ids::SETTINGS_DISPLAY_MODE_HINT),
                hint,
                palette.text_muted,
                11.0,
                400,
            ...
            for index in 0..SETTINGS_ZONE_DISPLAY_MODE_COUNT {
                ...
                let label_id = match mode {
                    ZoneDisplayMode::Hover => bentodesk_style::i18n_zh_cn::ids::ZONE_MODE_HOVER,
                    ZoneDisplayMode::Always => bentodesk_style::i18n_zh_cn::ids::ZONE_MODE_ALWAYS,
                    ZoneDisplayMode::Click => bentodesk_style::i18n_zh_cn::ids::ZONE_MODE_CLICK,
                };
                let label = settings_zone_display_mode_radio_label_rect(viewport, scroll, index);
                self.draw_text_no_wrap_with_style(
                    bentodesk_style::t(label_id),
                    label,
                    title_color,
