...
/// `--blur-expanded: blur(24px) saturate(170%)`), but native keeps one baked
...
/// `saturate(130%)`. The shared native backdrop is re-baked when theme polarity
...
pub(super) fn lerp_rect_clamped(
    from: bentodesk_style::Rect,
    to: bentodesk_style::Rect,
    progress: f32,
) -> bentodesk_style::Rect {
...
pub(super) fn item_icon_slots_for_card(
    ...
    is_wide: bool,
    scale: f32,
) -> (bentodesk_style::Rect, bentodesk_style::Rect) {
    let size = if is_wide {
