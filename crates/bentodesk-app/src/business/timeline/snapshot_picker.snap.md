...
- **Behaviour:** `createEffect` on `isSnapshotPickerOpen()` → `setLoading(true)` + `ipc.listSnapshots()` + `setConfirmDeleteId(null)`. Item Load → `ipc.loadSnapshot(id)` then close picker. Delete first click sets `confirmDeleteId(id)`; Yes confirms → `ipc.deleteSnapshot(id)` + filter list; No clears `confirmDeleteId`. Open-Timeline button closes picker + opens timeline. Escape closes. Backend dep: `bentodesk-backend::timeline::{list_snapshots, load_snapshot, delete_snapshot}` (T-089). Dispatcher hooks: open via `Command::ShowWindow(WindowKind::SnapshotPicker)`, close via `Command::HideWindow(WindowKind::SnapshotPicker)`. Open-Timeline button issues `Command::HideWindow(WindowKind::SnapshotPicker)` followed by `Command::ShowWindow(WindowKind::Timeline)`.
