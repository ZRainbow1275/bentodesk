...
fn item_card_chrome_accepts_explicit_active_palette() {
    ...
    assert_eq!(
        chrome.card_radius,
        BorderRadius::all(bentodesk_style::tokens::RADIUS.card)
    );
    // Normal bg is the Tauri `--surface-subtle` (white @ 0.03), not the
    // warm `surface_alt @ 0.46`.
    assert_eq!(
        chrome.normal_background,
        bentodesk_style::tokens::PALETTE_DARK.surface_subtle
    );
    assert_eq!(
    ...
    assert_eq!(
        chrome.text,
        bentodesk_style::tokens::PALETTE_DARK.text_secondary
    );
    assert_eq!(
        chrome.icon_text,
        bentodesk_style::tokens::PALETTE_DARK.text_primary
    );
}
...
fn item_card_chrome_uses_tauri_card_radius_token() {
    ...
    let palette = bentodesk_theme::current().palette;
    ...
    let chrome = ItemCardChrome::from_tokens(
        ...
        bentodesk_style::tokens::PALETTE_DARK.text_secondary,
        bentodesk_style::tokens::PALETTE_DARK.text_primary,
        bentodesk_style::tokens::PALETTE_DARK.surface_hover,
        bentodesk_style::tokens::PALETTE_DARK.border_hover,
    );
...
fn item_card_chrome_icon_text_uses_live_tauri_token() {
    ...
    let light = ItemCardChrome::from_tokens(
        palette,
        radius::DEFAULT,
        bentodesk_style::tokens::PALETTE_LIGHT.surface_subtle,
        bentodesk_style::tokens::PALETTE_LIGHT.text_secondary,
        bentodesk_style::tokens::PALETTE_LIGHT.text_primary,
        bentodesk_style::tokens::PALETTE_LIGHT.surface_hover,
        bentodesk_style::tokens::PALETTE_LIGHT.border_hover,
    );
