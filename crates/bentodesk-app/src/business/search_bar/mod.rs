...
//! shell queries `bentodesk_backend::search`, and the renderer reads real
...
/// One search-result row delivered by `bentodesk-backend::search`.
...
mod tests {
    ...
    fn search_bar_chrome_accepts_explicit_active_palette() {
        let mut palette = bentodesk_theme::current().palette;
