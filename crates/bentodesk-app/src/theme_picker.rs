...
//! - **Spec §8 (no new crate deps).** Uses only `bentodesk-style` tokens +
...
use bentodesk_style::tokens::SPACING;
use bentodesk_style::{Color, Rect, StringId, i18n_zh_cn::ids};
...
pub fn appearance_layout(origin: Point, inner_w: f32) -> AppearanceLayout {
    ...
    // Match Tauri's single 36×28 colour input. Native reuses the existing
