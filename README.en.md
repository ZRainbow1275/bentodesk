<div align="center">
  ...
  **A refined, next-generation bento-box desktop organizer for Windows, powered by Rust.**
...
Files on the desktop stay visible and remind us what still needs attention. Put everything several folders deep, and unfinished work is easily forgotten. Leave every project, document, and shortcut in view, and the desktop itself becomes a distraction.

BentoDesk adds a set of Zones that can collapse, expand, and form Stacks. Like sections in a bento box, they keep different kinds of work apart. A Zone stays as a quiet capsule until it is needed. Windows still owns the files; BentoDesk organizes, presents, and restores them.

Version 2.0 is rebuilt with Rust and the native Windows graphics stack. It ships without Tauri, WebView2, Chromium, or a Node.js runtime.
...
Settings is a native, draggable, scrollable, non-topmost window. It includes light and dark themes, accent colors, performance controls, startup options, backup, encryption, and updater settings. The first release defaults to Simplified Chinese and can switch to English in Settings.
...
These figures were recorded from one isolated Windows x64 machine and describe this candidate only:

| Metric | Result |
| --- | ---: |
| Release EXE | 2,518,528 bytes (2.40 MiB) |
| Strict scene | 5 Zones / 50 items / 1 process |
| Private Bytes at t30 | 17.43 MiB |
| Private Bytes at t60 | 17.33 MiB |
| Full Zone expand / collapse | 234 ms / 234 ms |
| Frame interval median / p95 | 16 ms / 16 ms |

The measurement includes only the BentoDesk process. Themes, window layering, motion, batch layout, and persistence were also checked in real runtime scenarios.
...
```text
bento-nano-shell      Process entry, Win32 message routing, system integration
bento-nano-app        Application state, interaction, render projection
bento-nano-backend    Settings, plugins, rules, recovery, updater
bento-nano-platform   D2D/DWrite/DComp/WIC/Shell/OLE boundaries
bento-nano-zone       Zone domain model
bento-nano-style      Themes, typography, and visual tokens
```

The internal `bento-nano-*` crate names preserve proven dependency boundaries. The user-facing product, executable, and version are BentoDesk 2.0.
...
Code, tests, translations, themes, plugins, and documentation are welcome. Issues and pull requests may be written in Chinese or English.

Before submitting, run at least the checks relevant to your change and state the boundary of real verification. Changes involving file moves, plugins, updates, or recovery should also describe data-integrity and rollback behavior.

## Thanks

Thanks to GPT 5.6 Sol for collaboration during the engineering reshape, to [Tibo](https://x.com/thsottiaux) for product inspiration, and to the Linux Do community for ongoing discussion, testing, and feedback.
