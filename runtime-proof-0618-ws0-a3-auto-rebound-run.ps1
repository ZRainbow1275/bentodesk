...
$stateDir = Join-Path $nano 'runtime-proof-0618-ws0-a3-auto-rebound-state'
$proofDir = Join-Path $nano 'runtime-proof-0618-ws0-a3-auto-rebound-try'
$proofExe = Join-Path $proofDir 'bento-nano-shell-a3-proof.exe'
...
function Append-Utf8NoBomLine {
  ...
  $encoding = New-Object System.Text.UTF8Encoding($false)
  [System.IO.File]::AppendAllText($Path, "$Text`n", $encoding)
}
...
Get-ChildItem -LiteralPath $proofDir -File -ErrorAction SilentlyContinue |
  Where-Object { $_.Name -ne 'runtime-proof-0618-ws0-a3-auto-rebound-run.ps1' } |
...
function Analyze-ImageDelta([string]$beforePath, [string]$afterPath) {
  ...
  try {
    ...
    return [ordered]@{
      ...
      changed_ratio = if ($samples -gt 0) { [Math]::Round($changed / [double]$samples, 6) } else { 0.0 }
      mean_rgb_delta = if ($samples -gt 0) { [Math]::Round($sumDelta / [double]$samples, 4) } else { 0.0 }
    }
  } finally {
    $before.Dispose()
    $after.Dispose()
...
try {
  ...
  foreach ($m in [regex]::Matches($stderrAll, '(?m)^anim_state: .*$')) {
    ...
    Append-Utf8NoBomLine $stateDumpPath ($row | ConvertTo-Json -Compress -Depth 8)
  }

  $hoverEnterRows = @($stateRows | Where-Object {
    $_.phase -eq 'hover_enter_armed' -and $_.hovered_zone -eq '4' -and $_.active_drag -eq 'none' -and $_.stack_bloom_anchor -eq 'none' -and $_.hover_scheduler_pending -eq 'true'
  })
  $expandRows = @($stateRows | Where-Object { $_.phase -eq 'hover_expand_fired' -and $_.hover_scheduler_expanded -eq '4' })
  $hoverLeaveRows = @($stateRows | Where-Object {
    $_.phase -eq 'hover_leave_armed' -and $_.hovered_zone -eq 'none' -and $_.active_drag -eq 'none' -and $_.hover_scheduler_expanded -eq '4' -and $_.hover_scheduler_pending -eq 'true'
  })
  $collapseRows = @($stateRows | Where-Object { $_.phase -eq 'hover_collapse_fired' -and $_.hover_scheduler_expanded -eq 'none' })
  $settledRows = @($stateRows | Where-Object {
    $_.phase -eq 'hover_collapse_settled' -and $_.hover_scheduler_expanded -eq 'none' -and $_.pill_anim_zone -eq 'none'
  })

  $hoverEnter = $hoverEnterRows | Select-Object -First 1
  $expandFired = $expandRows | Select-Object -First 1
  $hoverLeave = $hoverLeaveRows | Select-Object -First 1
  $collapseFired = $collapseRows | Select-Object -First 1
  $collapseSettled = $settledRows | Select-Object -Last 1

  $expandAfterEnterMs = if ($hoverEnter -and $expandFired) { [int]$expandFired.now_ms - [int]$hoverEnter.now_ms } else { $null }
  $collapseAfterLeaveMs = if ($hoverLeave -and $collapseFired) { [int]$collapseFired.now_ms - [int]$hoverLeave.now_ms } else { $null }
  $settledAfterLeaveMs = if ($hoverLeave -and $collapseSettled) { [int]$collapseSettled.now_ms - [int]$hoverLeave.now_ms } else { $null }

  $baselinePath = Join-Path $proofDir '03-expanded-00-before-collapsed.png'
  $openStablePath = Join-Path $proofDir '07-expanded-04-open-stable.png'
  $closeStablePath = Join-Path $proofDir '11-expanded-08-close-stable.png'
  $openDelta = Analyze-ImageDelta $baselinePath $openStablePath
  $closeDelta = Analyze-ImageDelta $baselinePath $closeStablePath
  $pixelAssertions = [ordered]@{
    baseline_vs_open_stable = $openDelta
    baseline_vs_close_stable = $closeDelta
  }
  Write-Utf8NoBom $pixelAssertionsPath ($pixelAssertions | ConvertTo-Json -Depth 8)
  ...
  $openedVisibly = $openDelta.changed_ratio -ge 0.002 -and $openDelta.mean_rgb_delta -ge 0.35
  $closedBackToPill = $closeDelta.changed_ratio -le ([double]$openDelta.changed_ratio * 0.35) -and $closeDelta.mean_rgb_delta -le ([double]$openDelta.mean_rgb_delta * 0.35)
  $a3Accepted = $zone4StillUnstacked `
    -and $zone4Before `
    -and $zone4AfterHover `
    -and ($allFramesSaved -eq 0) `
    -and $noWriteDuringHover `
    -and ($hoverEnterRows.Count -ge 1) `
    -and ($expandRows.Count -ge 1) `
    -and ($hoverLeaveRows.Count -ge 1) `
    -and ($collapseRows.Count -ge 1) `
    -and ($settledRows.Count -ge 1) `
    -and ($null -ne $expandAfterEnterMs) `
    -and ($null -ne $collapseAfterLeaveMs) `
    -and ($null -ne $settledAfterLeaveMs) `
    -and ($expandAfterEnterMs -ge 0) `
    -and ($collapseAfterLeaveMs -ge 0) `
    -and ($collapseAfterLeaveMs -le 1000) `
    -and ($settledAfterLeaveMs -le 1200) `
    -and $openedVisibly `
    -and $closedBackToPill `
  ...
  $summary = [ordered]@{
    status = if ($a3Accepted) { 'ok' } else { 'failed' }
    stage = 'completed'
    exe = $proofExe
    ...
    a3_auto_rebound = [ordered]@{
      ...
      hover_enter_seen = [bool]($hoverEnterRows.Count -ge 1)
      expand_fired_seen = [bool]($expandRows.Count -ge 1)
      hover_leave_seen = [bool]($hoverLeaveRows.Count -ge 1)
      collapse_fired_seen = [bool]($collapseRows.Count -ge 1)
      collapse_settled_seen = [bool]($settledRows.Count -ge 1)
      expand_after_enter_ms = $expandAfterEnterMs
      collapse_after_leave_ms = $collapseAfterLeaveMs
      settled_after_leave_ms = $settledAfterLeaveMs
      collapse_within_1s = [bool]($null -ne $collapseAfterLeaveMs -and $collapseAfterLeaveMs -le 1000)
      settled_within_1_2s = [bool]($null -ne $settledAfterLeaveMs -and $settledAfterLeaveMs -le 1200)
      opened_visibly = [bool]$openedVisibly
      closed_back_to_pill = [bool]$closedBackToPill
      ...
      measurement_boundary = 'State timing comes from proof-only anim_state rows; screenshots prove open-stable differs from collapsed baseline and close-stable returns near baseline.'
    }
    animation_state = [ordered]@{
      state_dump_jsonl = 'state-dumps.jsonl'
      anim_state_log_count = [int]$stateRows.Count
      hover_enter_count = [int]$hoverEnterRows.Count
      expand_fired_count = [int]$expandRows.Count
      hover_leave_count = [int]$hoverLeaveRows.Count
      collapse_fired_count = [int]$collapseRows.Count
      collapse_settled_count = [int]$settledRows.Count
      hover_enter = $hoverEnter
      expand_fired = $expandFired
      hover_leave = $hoverLeave
      collapse_fired = $collapseFired
      collapse_settled = $collapseSettled
    }
  ...
  if ($proc) {
    try {
      if ([string]::IsNullOrEmpty($stderrAll) -and (Test-Path -LiteralPath $stderrPath)) {
        $stderrAll = Get-Content -LiteralPath $stderrPath -Raw
      }
      if ([string]::IsNullOrEmpty($stdoutAll) -and (Test-Path -LiteralPath $stdoutPath)) {
        $stdoutAll = Get-Content -LiteralPath $stdoutPath -Raw
      }
    } catch {}
  }
  $summary = [ordered]@{
