...
$stateDir = Join-Path $nano 'runtime-proof-0618-animation-state-arbitration-state'
$proofDir = Join-Path $nano 'runtime-proof-0618-animation-state-arbitration-try'
$proofExe = Join-Path $proofDir 'bento-nano-shell-animation-proof.exe'
...
$oldProofProcess = Get-Process -Name 'bento-nano-shell-animation-proof' -ErrorAction SilentlyContinue
if ($oldProofProcess) {
  throw 'bento-nano-shell-animation-proof is already running; stop the old proof process before rerunning'
}
...
function Save-DragFrame($win, [string]$path, [string]$previousPath, [double]$scale, [hashtable]$scanRect) {
  ...
  while ($attempt -lt 4) {
    $attempt++
    Request-Paint $win 100
    Save-WindowShot $win $path | Out-Null
    if (-not $previousPath -or -not (Test-Path -LiteralPath $previousPath)) {
      return [ordered]@{ attempts = $attempt; delta = $null }
    }
    $delta = Analyze-ImageDelta $previousPath $path $scale $scanRect
    if ($delta.changed_ratio -ge 0.003 -and $delta.mean_rgb_delta -ge 0.4) {
      return [ordered]@{ attempts = $attempt; delta = $delta }
    }
...
try {
  ...
  foreach ($point in $dragPath) {
    ...
    [void]$clicks.Add((Send-ClientMessage $main ([NativeProof0618Anim]::WM_MOUSEMOVE) $point[0] $point[1] 'send' 70))
  ...
  $zoneDragStates = @($stateRows | Where-Object { $_.phase -eq 'zone_drag_live' })
  ...
  $summaryStatusOk = $hoverBloomSeen `
    -and $zone4NormalHoverSeen `
    -and $stateArbitrationIdle `
    -and $itemStateArbitrationIdle `
    -and $tickSkipSeen `
    -and $releaseCleared `
    -and $motionMonotonic `
